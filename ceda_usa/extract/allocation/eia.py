...
GCS_EIA_DIR = os.path.join(GCS_CEDA_INPUT_DIR, "EIA_EnergyPrice")
...
def load_propane_annual_avg_residential_price() -> float:
    ...
    """
    tbl = load_from_gcs(
        os.path.join(GCS_EIA_DIR, "U.S._Propane_Residential_Price.csv"),
        local_dir=IN_DIR,
...
def load_heating_oil_annual_avg_residential_price() -> float:
    ...
    """
    tbl = load_from_gcs(
        os.path.join(GCS_EIA_DIR, "U.S._No._2_Heating_Oil_Residential_Price.csv"),
        local_dir=IN_DIR,
        loader=lambda pth: pd.read_csv(
            pth,
            skiprows=4,
