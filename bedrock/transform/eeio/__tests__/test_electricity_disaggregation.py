        ...
        try:
            with mock.patch(
                ...
                with mock.patch(
                    ...
                    get_base_year_eia_purchaser_allocation.cache_clear()
                    _derive_post_reallocation_checkpoint_for_disagg.cache_clear()
                    with dollar_industrial_weights():
                        alloc = get_base_year_eia_purchaser_allocation()
...
def test_import_fd_column_uses_child_sum_when_aggregate_missing(
    ...
    monkeypatch.setattr(
        'bedrock.transform.eeio.electricity_gtd_allocation.get_base_year_eia_purchaser_allocation',
        lambda: alloc,
    )
    Y = pd.DataFrame(
        0.0,
