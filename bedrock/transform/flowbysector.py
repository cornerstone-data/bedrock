...
class FlowBySector(_FlowBy):
    ...
    def generateFlowBySector(
        ...
        **kwargs,
        ...
        method_config = common.load_yaml_dict(
        ...
        if 'cache' in method_config:
            log.warning('Config key "cache" for %s about to be overwritten', method)

        method_config['cache'] = {}
        for source_name, config in to_cache.items():
            method_config['cache'][source_name] = get_flowby_from_config(
                name=source_name,
                config={
                    **method_config,
    ...
    def display_tables(
        ...
        if display_tables is None:
            log.error(
                'Cannot generate display tables, since no configuration '
        ...
        table_dict = {
            table_name: (
                self.select_by_fields(table_config.get('selection_fields'))
        ...
        try:
            ...
            log.warning(
                f'Permission to write display tables for '
...
def collapse_FlowBySector(
    ...
    fbs = getFlowBySector(
        methodname, fbsconfigpath, download_FBAs_if_missing, download_FBS_if_missing
    )
