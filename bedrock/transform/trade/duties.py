...
from bedrock.transform.trade.utilities import consolidate_import_activities
...
def _census_flow_by_activity(year: int, flow_name: str) -> pd.Series:
    ...
    # ⚠️ The same family consolidation the Trade FBS applies (#763, which
    # generalised #702's vehicle case). The rate below is a ratio of two mapped
    # Census flows and it multiplies the Trade_Imports FBS's own mass, so both
    # sides have to fold families the same way or the rate is built on one
    # commodity axis and applied to another.
    sub = consolidate_import_activities(sub)
    return (
