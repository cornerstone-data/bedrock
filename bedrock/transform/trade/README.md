...
Crosswalks: `Sector_Crosswalk_Census_USATrade.csv` (shared by exports and imports) and direction-specific IEA files `Sector_Crosswalk_BEA_IEA_exports.csv` / `Sector_Crosswalk_BEA_IEA_imports.csv` (`SectorSourceName` `BEA_2017_Code`). `Trade_Exports_<year>` selects `BEA_IEA_exports`; `Trade_Imports_<year>` selects `BEA_IEA_imports`. Non-financial IEA rows are identical across both files. IEA rows are deepest `TypeOfService` codes whose label names the Detail commodity (or a small family the type spans). Parent totals are omitted when children are mapped (`AllTypesOfService`, `Transport`, `ChargesForTheUseOfIpNie`, `TelecomCompAndInfo`, `OtherBusiness`, `ProfMgmtConsult`, `Travel`, `Financial`, `FinExplicitAndOth`). Crosswalk membership is not chosen by closing 2017 Use/Supply cells. Methods include `BEA_detail_commodity_target.yaml` so FBS output stays on BEA 2017 Detail commodities. Calling FBAs are `TECHNOSPHERE_FLOW` with empty `ActivityConsumedBy`; activity sets select by `FlowName` (and IEA exclusions). Both directions take their 1:m weights from a published 2017 column via `selection_fields`: exports from Use `F04000` on `BEA_Detail_Use_SUT`, imports from Supply `MCIF` on `BEA_Detail_Supply` (#701 replaced the export side’s same-year `T007`; see 1:m weights under IEA Crosswalk revisions).
...
| --- | --- | --- | --- |
| `Financial` | 1:m → `52A000`, `522A00`, `523A00`, `523900`; split by published 2017 `F04000` | *unmapped* | Parent total (~$38 B imports) has no SUT `MCIF` home; mapping it dumps mass onto `52A000` (5.7× SUT). |
...
**Export financial rule:** map parent `Financial` only; the proportional split uses published 2017 `F04000` weights among the four Detail codes. **Import financial rule:** map `FinFisim` → `52A000` only (direct 1:1; no proportional step).
...
- **Census residual NAICS (`*X` / `*XX`).** Census USA Trade publishes suppressed-detail residuals alongside digit-6 NAICS (2017 exports: `33641X` 120,967 M, `31181X`, `31135X`, `31131X`, `11211X`, `1123XX`; imports: the same set except `33641X`). From 2022 Census publishes `1121XX` instead of `11211X`; the Crosswalk aliases `1121XX` to the same Detail targets (`1121A0` / `112120`). `census_usatrade_parse` keeps `\d{6}|\d{5}X|\d{4}XX`. `Sector_Crosswalk_Census_USATrade` maps each residual 1:m onto the Detail commodities of that family (`33641X` → `336411`/`336412`/`336413`/`336414`/`33641A`); export splits use published 2017 `F04000`, import splits use frozen 2017 Supply `MCIF`. ⚠️ **On exports the aerospace leaves are folded onto `33641X` first** (#701): Census names a leaf for only 10.7% of 2017 aerospace exports and the mix of that 10.7% is 39.6% away from the published export mix, so splitting the residual around those rows leaves the bias in place. Imports keep their leaves — there is no `33641X` on that side. Dropping residuals at parse left those Crosswalk rows unreachable and understated aerospace exports by ~$121 B.
...
- **Outstanding goods-allocation hypothesis (`#670`).** Large non-`MISS` gaps in some directly mapped goods families suggest the NAICS-based Census goods Crosswalk may not fully reproduce BEA's product-level foreign-trade allocation. The aerospace cluster was primarily a dropped `33641X` residual, not an HTS surface mismatch, and after #701 re-weighted and consolidated the family every one of its five leaves sits at **1.06×** 2017 F040 — one uniform family-level gap rather than a split error. Track remaining evidence in [#670](https://github.com/cornerstone-data/bedrock/issues/670).
- **Utilities — electricity physical override (#668).** `221100` uses EIA Electric Power Annual Table 2.14 national Canada+Mexico MWh × same-year Census HS `2716000000` unit value (USD/MWh), layered as `EIA_ElectricPowerAnnual` on the existing Trade methods. Overlay dollars track physical trade, not published SUT F040/MCIF (export F040 remains much larger than official electric-energy merchandise). Unit values live in `extract/census/data/hs2716_electricity_unit_value.csv`; refresh with `refresh_hs2716_electricity_unit_value_csv()` in `bedrock.extract.census.Census_USATrade` (Census HS API shortcut until a full HS FBA extract exists). `221200` (natural gas distribution) and `221300` (water, sewage, and other systems) stay unmapped: SUT shows export-only stubs with zero MCIF; measured gas merchandise belongs on `211000`, and neither cell has a Census/IEA or physical series that maps to the utility commodity (see #668).
- **Couriers `492000`.** No IEA courier TypeOfService. UK/ONS bilateral notes put BEA courier inside **air transport** (postal in other transport). Both IEA Crosswalks map `TransportAirFreight` 1:m → `481000` / `492000`, **Since #771 both rows are anchored on their published 2017 value and moved by that category's growth**, so couriers is exact at 2017 (exports 9,411 M, imports 44 M) and the only open claim is the mover. Graded on published summary `487OS` — which is `492000` + `48A000` and nothing else — the air-freight mover lands at **1.68% mean absolute error 2018-2024**, against 17.8% for the next best category and 66.1% for the pre-#771 `T007` split; see `bedrock/analysis/nowcasting/trade_data/courier_air_freight.py`. ⚠️ On the **import** side a frozen level scores better than the mover (13.8% against 43.8%) but the largest error is 35 M USD, so the uniform bridge rule is kept. `TransportPostal` stays `491000` only.
...
- **Family consolidation.** Both directions fold Census activities onto a parent before attribution so the 1:m weight, not Census's own within-family split, decides where mass lands. Imports fold **every** four-digit family with more than one published leaf (`consolidate_import_activities_fba`, 48 families, 165 generated crosswalk rows carrying `Note = FAMILY_PARENT`, written by `bedrock.utils.mapping.write_census_family_parents`) — **only where Census's family total is within 25% of BEA's**, because a family whose level is wrong would spread one leaf's level error over siblings that were fine; the 13 excluded families are #670's; exports fold vehicles and aerospace only (`consolidate_export_activities_fba`). ⚠️ `duties.map_census_import_flow_to_detail` applies the **same** import consolidation — the duty rate is a ratio of two mapped Census flows multiplying the FBS's own mass, so folding on one side only builds the rate on one commodity axis and applies it to another.
- **1:m weights** -- exports: published 2017 Use `F04000`; imports: frozen 2017 Supply `MCIF` until total uses exist ([#729](https://github.com/cornerstone-data/bedrock/issues/729)). ⚠️ #729’s same-year `T007` was **withdrawn** on the export side in #701: a supply weight answers "who produced it" and an export split needs "who exported it", which differ by a factor of eight inside aerospace. Graded on the benchmark panel, same-year `T007` puts 13.7% (2007→2012) and 20.9% (2012→2017) of family export dollars on the wrong leaf against the prior benchmark’s export mix at 3.2% and 4.7% — see `analysis/nowcasting/trade_data/export_split_weight.py`. Equal shares remain a README candidate only.
- **Financial services** -- direction-specific Crosswalks (`BEA_IEA_exports` vs `BEA_IEA_imports`). Exports: parent `Financial` 1:m by published 2017 `F04000`. Imports: `FinFisim` → `52A000` only. Do not map parent `Financial` or explicit `Fin*` leaves onto `MCIF`.
