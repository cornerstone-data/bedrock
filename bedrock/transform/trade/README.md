...
- **Export `MISS` >= 1 B USD.** `492000` couriers (9,411 M), `550000` management of companies (4,296 M), `531ORE` other real estate (3,274 M), `221100` electric power (2,744 M), `311810` bread and bakery (1,764 M), `722110` full-service restaurants (1,177 M), `722211` limited-service restaurants (1,130 M). Rule: no Census NAICS-6 and no mapped IEA type hits that Detail commodity.
- **Import `MISS` >= 1 B USD besides `S00300`.** `311810` (4,710 M), `221100` (2,431 M), `1121A0` beef cattle (1,659 M), `561300` employment services (1,588 M). Same rule.
...
- **Outstanding goods-allocation hypothesis (`#670`).** Large non-`MISS` gaps in directly mapped goods families (for example `336*`) suggest the NAICS-based Census goods Crosswalk may not reproduce BEA's product-level foreign-trade allocation and valuation/reconciliation workflow. Track evidence and follow-up in [#670](https://github.com/cornerstone-data/bedrock/issues/670).
...
- **Goods coverage** -- Census FBA has no `1121*` activity (`1121A0` import hole, 1,659 M MCIF; reachable only via `11211X` 1:m split). `311824` maps to `3118A0` per BEA's NAICS-to-Detail crosswalk (dry pasta/dough/flour mixes, not bakery). `980000` (low-value shipments) stays omitted; no Detail sector.
