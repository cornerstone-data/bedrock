...
- **Couriers `492000`.** No IEA courier TypeOfService. UK/ONS bilateral notes put BEA courier inside **air transport** (postal in other transport). Both IEA Crosswalks map `TransportAirFreight` 1:m → `481000` / `492000`, split by frozen 2017 Use `F04000` (exports) or Supply `MCIF` (imports). That is the native proportional rule (~18% of air-freight mass to `492000` on exports given F040 weights); it does not invent a courier share beyond SUT weights. `TransportPostal` stays `491000` only.
