...
def return_primary_flow_column(self: FB, flowtype: str = 'FBA') -> str:
    ...
    if self[f'{flow}ProducedBy'].isnull().all():
        ...
        log.error(
            f'Could not determine primary {flow} column as there are '
            f'values in both {flow}ProducedBy and {flow}ConsumedBy'
        )
    return primary_column


def load_prepare_clean_source(self: 'FB', download_sources_ok: bool = True) -> 'FB':
    """
    Add doc string
    """
    try:
    ...
    clean_fbs = get_flowby_from_config(
        ...
        config={
            **{
                ...
                if k in self.config['method_config_keys'] or k == 'method_config_keys'
            },
...
def weighted_average(
    ...
    merged = fba.merge(
        ...
        left_on=[
            'PrimarySector',
            'temp_location' if 'temp_location' in fba else 'Location',
        ],
...
def substitute_nonexistent_values(
    ...
    log.info(
        'Substituting nonexistent values in %s with %s.', fb.full_name, other.full_name
    )
    ...
    merged = fb.merge(
        other,
        on=list(other.select_dtypes(include=['object', 'int']).columns),
        how='outer',
        suffixes=(None, '_y'),
    ).fillna({'FlowAmount': 0})
    ...
    for c in new_col_data:
        ...
        if merged[original_col].dtype != 'string':
            ...
            merged[original_col] = merged[original_col].fillna(merged[c])

    # reset group id and group total, drop columns
    merged = (
        merged.drop(merged.filter(regex='_y').columns, axis=1)
...
def estimate_suppressed_sectors_equal_attribution(
    ...
    fba3 = fba3.assign(Unattributed=fba3.FlowAmount.copy(), Attributed=0).assign(
        descendants=''
    )

    # drop rows that contain "&" and "-"
    fba3 = fba3.query(f"~{col}.str.contains('&')").query(f"~{col}.str.contains('-')")

    for level in [6, 5, 4, 3, 2]:
        descendants = (
            fba3.drop(columns='descendants')
            .loc[fba3[col].str.len() > level]
            .assign(parent=lambda x: x[col].str.slice(stop=level))
        ...
        fba3 = (
            ...
            .assign(
                descendants=lambda x: x.descendants.mask(
                    x.descendants == '', x.descendants_y
                ),
                Unattributed=lambda x: (x.Unattributed - x.descendant_flows).mask(
                    x.Unattributed - x.descendant_flows < 0, 0
                ),
                Attributed=lambda x: (x.Attributed + x.descendant_flows),
            )
    ...
    indexed = fba_m.set_index(
        ['n2', 'n3', 'n4', 'n5', 'n6', 'n7', 'location', 'category'],
    ...
    sector_len_indexed = unsuppressed[col].str.len()
...
def attribute_national_to_states(fba: FlowByActivity, **_: Any) -> FlowByActivity:
    ...
    hlp['Allocation'] = hlp['FlowAmount'] / hlp.groupby(
        ['SectorProducedBy', 'SectorConsumedBy'], dropna=False
    ...
    fba = pd.merge(  # type: ignore[assignment]
        ...
        (
            hlp[
                ['Location_merge', 'Location', 'SectorProducedBy', 'Allocation']
            ].rename(columns={'SectorProducedBy': 'SectorConsumedBy'})
        ),
        how='left',
        on=['Location_merge', 'SectorConsumedBy'],
    )
    fba = fba.assign(FlowAmount=lambda x: x['FlowAmount'] * x['Allocation']).drop(
        columns=['Allocation', 'Location_merge']
    )
...
def calculate_flow_per_person(
    ...
    clean_fbs function
    """
    bls = load_prepare_clean_source(fbs, download_sources_ok=download_sources_ok)
    ...
    fbs = (
        fbs.sector_aggregation()
        .aggregate_flowby()
        # ^^ handles updated industry specs
        .merge(
            bls.rename(columns={'FlowAmount': 'persons'})
            .groupby(cols)
            .agg({'persons': 'sum'})
            .reset_index(),
            how='inner',
        ...
        .assign(
            FlowAmount=lambda x: np.divide(
                x['FlowAmount'],
                x['persons'],
                out=np.zeros_like(x['persons']),
                where=x['persons'] != 0,
            )
        )
...
def define_parentincompletechild_descendants(
    ...
    '''
    fba = fba.query(f'{activity_col} != "31-33"').assign(descendants='')

    for level in [5, 4, 3]:
        descendants = (
            fba.drop(columns='descendants')
            .query(f'{activity_col}.str.len() > {level}')
            .assign(parent=lambda x: x[activity_col].str.slice(stop=level))
            .groupby(['Flowable', 'Location', 'parent'])
            .agg({'FlowAmount': 'sum', activity_col: ' '.join})
            .reset_index()
            .rename(
                columns={
                    activity_col: 'descendants',
    ...
    fba = fba.drop(columns='group_total').merge(
        (
            fba.groupby('group_id')
            .agg({'FlowAmount': "sum"})
