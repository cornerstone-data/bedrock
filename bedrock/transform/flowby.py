    ...
    if config.get('data_format') == 'FBA':
        ...
        return FlowBySector(  # todo: add convert_df_to_flowby=True?
            config['FBS_datapull_fxn'](
                config=config, external_config_path=external_config_path, full_name=name
            ),
            full_name=name,
            config=config,
        )
        ...
        log.critical(
            f'Unrecognized `config["data_format"] = '
...
class _FlowBy(pd.DataFrame):
        ...
        **kwargs: Any,
        ...
        for attribute in self._metadata:
            if not hasattr(self, attribute):
                ann_type = self.__annotations__.get(attribute)
                default = ann_type() if ann_type is not None else None
                super().__setattr__(
                    attribute,
                    getattr(
                        data,
                        attribute,
                        kwargs.pop(attribute, default),
                    ),
                )
        ...
        if isinstance(data, pd.DataFrame) and fields is not None:
            if convert_df_to_flowby and add_missing_columns:
                data = data.assign(
                    **{field: None for field in fields if field not in data.columns}
                )
            else:
            ...
            null_string_dict = {
                field: {
                    null: string_null
                    for null in ['nan', '<NA>', 'None', '', np.nan, pd.NA, None]
                }
                for field, dtype in fields.items()
                if dtype == 'object'
            }
        ...
        if (
            ...
            and column_order is not None
            and convert_df_to_flowby
        ):
            data = data[
                [c for c in column_order if c in data.columns]
                + [c for c in data.columns if c not in column_order]
            ]
        super().__init__(data, *args, **kwargs)  # type: ignore[call-arg]

    @property
    def _constructor(self) -> type['_FlowBy']:
        return _FlowBy

    @property
    def _constructor_sliced(self) -> type['_FlowBySeries']:
    ...
    def __finalize__(  # type: ignore[misc]
        ...
        if method == 'merge':
            for attribute in self._metadata:
                object.__setattr__(
                    self, attribute, getattr(other.left, attribute, None)
                )
        ...
        if method == 'concat':
            ...
            for i, n in enumerate(
                getattr(other.objs[0], 'full_name', '').split(NAME_SEP_CHAR)
            ):
                if all(
                    n == getattr(x, 'full_name', '').split(NAME_SEP_CHAR)[i]
            ...
            _config = {
                k: v
                for k, v in getattr(other.objs[0], 'config', {}).items()
                if all(v == getattr(x, 'config', {}).get(k) for x in other.objs[1:])
            }
            object.__setattr__(self, 'full_name', _full_name)
            object.__setattr__(self, 'config', _config)
            for attribute in [
                x for x in self._metadata if x not in ['full_name', 'config']
            ]:
                object.__setattr__(
                    self, attribute, getattr(other.objs[0], attribute, None)
                )
        return self
    ...
    def convert_daily_to_annual(self: FB) -> FB:
        ...
        if any(self.Unit.str.endswith(tuple(daily_list))):
            log.info(
                f'Converting daily flows '
                f'{[unit for unit in self.Unit.unique() if any(x in unit for x in daily_list)]} '
                f'to annual'
            )
        return self.assign(
            FlowAmount=self.FlowAmount.mask(
                self.Unit.str.endswith(tuple(daily_list)), self.FlowAmount * 365
            ),
            Unit=self.Unit.apply(lambda x: x.split('/d', 1)[0]),
        )

    def standardize_units(self: FB, year: int | None = None) -> FB:
        exchange_rate = literature_values.get_Canadian_to_USD_exchange_rate(
            year or self.config['year']
        )
        ...
        with pd.option_context('future.no_silent_downcasting', True):
            standardized = (
                self.assign(Unit=self.Unit.str.strip())
                .merge(
                    conversion_table, how='left', left_on='Unit', right_on='old_unit'
                )
                .assign(
        ...
        if any(~standardized.Unit.isin(standardized_units)):
            ...
            log.warning(
                f'Some units in {standardized.full_name} not '
    ...
    def function_socket(self: FB, socket_name: str, *args: Any, **kwargs: Any) -> FB:
        ...
        if socket_name in self.config:
            if isinstance(self.config[socket_name], list):
                return reduce(
                    lambda fb, func: fb.pipe(func, *args, **kwargs),
    ...
    def convert_fips_to_geoscale(
        self: FB,
        target_geoscale: str | geo_scale | None = None,
        column: str = 'Location',
    ) -> FB:
        """
        ...
        elif target_geoscale == geo_scale.STATE:
            return self.assign(
                **{column: self[column].str.slice_replace(start=2, repl='000')}
            )
        elif target_geoscale == geo_scale.COUNTY:
            return self
        else:
    ...
    def select_by_fields(
        ...
        if selection_fields_resolved is None or selection_fields_resolved == 'null':
            return self

        selection_fields = selection_fields_resolved
        ...
        if 'PrimaryActivity' in selection_fields:
            self = self.add_primary_secondary_columns('Activity').drop(
                columns='SecondaryActivity'
            )
        if 'PrimarySector' in selection_fields:
            self = self.add_primary_secondary_columns('Sector').drop(
                columns='SecondarySector'
            )

        special_fields = {
            k: v for k, v in selection_fields.items() if k in ['Activity', 'Sector']
        }
        other_fields = {
            k: v for k, v in selection_fields.items() if k not in ['Activity', 'Sector']
        }
    ...
    def aggregate_flowby(
        self: FB,
        columns_to_group_by: list[str] | None = None,
        columns_to_average: list[str] | None = None,
        ...
        fb = self.drop(
            columns=[
                ...
                if c not in ['FlowAmount', *columns_to_average, *columns_to_group_by]
            ]
        )
        if len(self) == 0:
        ...
        aggregated = (
            fb.assign(
                **{f'_{c}_weighted': fb[c] * fb.FlowAmount for c in columns_to_average},
                **{
        ...
        aggregated = aggregated.assign(
            **{
                c: (aggregated[f'_{c}_weighted'] / aggregated[f'_{c}_weights'])
                for c in columns_to_average
            }
        ).drop(
            columns=(
        ...
        aggregated = aggregated.astype(
            {
                column: type
                for column, type in set(
                    [
                        *flowby_config['all_fba_fields'].items(),
        ...
        if 'group_total' in self.columns:
            aggregated = aggregated.assign(group_total=aggregated['FlowAmount'])
    ...
    def attribute_flows_to_sectors(
        ...
        assert isinstance(attribute_config, list)
        ...
        for index, step_config in enumerate(attribute_config):
            ...
            if index > 0:
                ...
                fb = (
                    ...
                    .function_socket(
                        'clean_fba_w_sec', attr=self.config, method=self.config
                    )
            ...
            if attribution_method == 'proportional':
                ...
                log.info(
                    f'Directly attributing {self.full_name} to sectors, '
                ...
                if all(fb.groupby('group_id')['group_id'].agg('count') == 1):
                    log.info(
                        f'No allocation needed for {fb.full_name} at '
                    ...
                    if step_config.get('attribution_method') is None:
                        log.warning(
                            f'No allocation method specified for '
                    ...
                    elif not all(fb.groupby('group_id')['group_id'].agg('count') == 1):
                        log.warning(
                            f'Allocation of {fb.full_name} includes '
            ...
            if validate and attribution_method not in [
                ...
                validation_fb = attributed_fb.assign(
                    validation_total=(
                        attributed_fb.groupby('group_id')['FlowAmount'].transform('sum')
                    )
                )
                if not np.allclose(
                    validation_fb.group_total,
                    validation_fb.validation_total,
                    equal_nan=True,
                ):
                    errors = validation_fb.query('validation_total != group_total')[
                        [
                            'group_id',
                            'ActivityProducedBy',
                            'ActivityConsumedBy',
                ...
                attsum = (
                    validation_fb[['group_id', 'validation_total']].drop_duplicates()
            ...
            attributed_fb = attributed_fb.function_socket('clean_fba_after_attribution')

            # Drop columns created for disaggregation, plus any
            # specified in the config file
            self = attributed_fb.drop(  # type: ignore[assignment]
                columns=[
                    'group_id',
                    'group_total',
                    'group_count',
    ...
    def activity_sets(self) -> list['_FlowBy']:
        ...
        for activity_set, activity_config in activities.items():
            ...
            if not child_df.empty:
                ...
                log.error(
                    f'Activity set {child_df.full_name} is empty. '
                    'Check activity set definition!'
                )

        if set(parent_df.row) - assigned_rows:
            log.warning(
                f'Some rows from {parent_df.full_name} not assigned '
    ...
    def harmonize_geoscale(
        ...
        if fill_cols and 'Location' in fill_cols:
            ...
            log.info(
                f'{other.full_name} is {other_geoscale}, while '
                f'{self.full_name} is {fb_geoscale}, so attributing '
                f'{other_geoscale} to {fb_geoscale}'
            )
            self = self.assign(temp_location=self.Location).convert_fips_to_geoscale(
                other_geoscale, column='temp_location'
            )
    ...
    def proportionally_attribute(
        ...
        if self.config.get('attribute_on') is None:
            ...
            for rank in ['Primary', 'Secondary']:
                ...
                counted = fb.assign(
                    group_count=(
                        fb.groupby(groupby_cols)['group_id'].transform('count')
                    )
                )
                directly_attributed = counted.query('group_count == 1').drop(
                    columns='group_count'
                )
                needs_attribution = counted.query('group_count > 1').drop(
                    columns='group_count'
                )
                ...
                non_zero_denominator = with_denominator.query('denominator != 0 ')
                unattributable = with_denominator.query('denominator == 0 ')
                ...
                if not unattributable.empty:
                    ...
                    if other_geoscale.aggregation_level < 5:
                        vlog.warning(
                            'This can occur when combining datasets '
                            'at a sub-national level when activities '
                            'do not align for some locations.'
                        )
                        vlog.warning(
                            f'{other.full_name} is at geoscale '
            ...
            fb = pd.concat(  # type: ignore[assignment]
                [directly_attributed, proportionally_attributed], ignore_index=True
            )

        # else attribute on column specified in the FBS yaml
        else:
            attribute_cols: list[str] = self.config.get('attribute_on', [])
            ...
            merged_with_denominator = merged.assign(
                denominator=(
                    merged.groupby('group_id')['FlowAmount_other'].transform('sum')
                )
            )

            non_zero_denominator = merged_with_denominator.query('denominator != 0 ')
            unattributable = merged_with_denominator.query('denominator == 0 ')
            ...
            if not unattributable.empty:
                vlog.warning(
                    ...
                    sorted(
                        set(
                            zip(
                                unattributable.SectorProducedBy.fillna('N/A'),
            ...
            fb = non_zero_denominator.assign(
                FlowAmount=lambda x: (x.FlowAmount * x.FlowAmount_other / x.denominator)
            )
        ...
        if fill_col is not None:
            log.info(
                f'Replacing {fill_col} values in primary data source '
    ...
    def multiplication_attribution(self: 'FB', other: 'FlowBySector') -> '_FlowBy':
        ...
        log.info(
            f'Multiplying flows in {self.full_name} {self_units} by'
        ...
        fb = merged.assign(
            FlowAmount=lambda x: (x.FlowAmount * x.FlowAmount_other)
        ).drop(
            columns=[
                'PrimarySector_other',
                'Location_other',
        ...
        if len(fb_null) > 0:
            # implode the location data to shorten warning message
            fb_null = fb_null.groupby(  # type: ignore[assignment]
        ...
        # converted
        rate = None
        non_rate_col: str | None = None
        if fb['Unit'].str.contains('/').all():
            rate = 'Unit'
            non_rate_col = 'Unit_other'
    ...
    def division_attribution(self: 'FB', other: 'FlowBySector') -> '_FlowBy':
        """
        This method takes flows from the calling FBA which are mapped to
        ...
        log.info(
            f'Dividing flows in {self.full_name} {self_units} by'
            f' {other.full_name} {other_units}'
        )
        fb_geoscale, other_geoscale, fb, other = self.harmonize_geoscale(other)
    ...
    def assign_temporal_correlation(
        ...
        fbs = esupy.dqi.adjust_dqi_scores(
            fbs, abs(fbs['Year'] - target_year), 'TemporalCorrelation'
        )
    ...
    def assign_geographic_correlation(
        ...
        **_kwargs: Any,
        ...
        if (
            'GeographicalCorrelation' in fbs
            and (fbs['GeographicalCorrelation'] == 0).all()
        ):
        ...
        fbs['GeographicalCorrelation'] = (
            fbs['GeographicalCorrelation'] - fips_number_key[target_geoscale]
        )
        fbs = esupy.dqi.adjust_dqi_scores(
            fbs, fbs['GeographicalCorrelation'], 'GeographicalCorrelation'
        )
        return fbs
    ...
    def add_primary_secondary_columns(
        ...
        if f'{col_type}ProducedBy' not in self or f'{col_type}ConsumedBy' not in self:
            log.error(
                f'Cannot add Primary{col_type} or Secondary{col_type} '
            ...
            if 'primary_action_type' in self.config:
                ...
                secondary_action_type = (
                    'Consumed' if primary_action_type == 'Produced' else 'Produced'
                )

                fb = self.assign(
                    **{
                        f'Primary{col_type}': self[
                            f'{col_type}{primary_action_type}By'
                        ],
                        f'Secondary{col_type}': self[
                            f'{col_type}{secondary_action_type}By'
                        ],
                    }
                )
...
class _FlowBySeries(pd.Series):
    ...
    @property
    def _constructor(self) -> type['_FlowBySeries']:
        return _FlowBySeries

    @property
    def _constructor_expanddim(self) -> type['_FlowBy']:
        return _FlowBy
