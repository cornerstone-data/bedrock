...
from bedrock.transform.iot import nowcast_mask as nm
...
def test_fixed_cells_stay_at_seed_under_row_target_shrink() -> None:
    """Tier-1 holds: fixed cells unchanged; free cells absorb (#899 cell hold)."""
    seed = _seed()
    commodities = list(seed.index)
    industries = list(seed.columns)
    # Pick a nonzero seed cell and freeze it.
    irow, jcol = 3, 5
    while seed.iloc[irow, jcol] == 0.0:
        jcol += 1
    fixed = pd.DataFrame(False, index=commodities, columns=industries)
    fixed.iloc[irow, jcol] = True
    frozen_value = float(seed.iloc[irow, jcol])

    row_t = seed.sum(axis=1) * 0.80  # shrink rows
    col_t = seed.sum(axis=0) * 0.80
    ...
    def _mask(
        ...
        *,
        ...
        del block, year, panel, trade_electricity_pin
        return fixed
    ...
    with pytest.MonkeyPatch.context() as patch:
        ...
        patch.setattr(nm, 'fixed_value_mask', _mask)
        result = fi.fit_interior(2023, seed=seed, trade_electricity_pin=True)

    assert float(result.interior.iloc[irow, jcol]) == pytest.approx(
        frozen_value, abs=1e-6
    )
    # Free mass on that row moved (row target shrank).
    free_row_seed = float(seed.iloc[irow].sum()) - frozen_value
    free_row_fit = float(result.interior.iloc[irow].sum()) - frozen_value
    assert free_row_fit != pytest.approx(free_row_seed, abs=1.0)


def test_frozen_mass_above_target_raises() -> None:
    seed = _seed()
    commodities = list(seed.index)
    industries = list(seed.columns)
    irow, jcol = 2, 4
    ...
    fixed = pd.DataFrame(False, index=commodities, columns=industries)
    fixed.iloc[irow, jcol] = True
    frozen = float(seed.iloc[irow, jcol])

    row_t = seed.sum(axis=1) * 1.1
    row_t.iloc[irow] = frozen * 0.5  # target below frozen mass
    col_t = seed.sum(axis=0) * 1.1
    ...
    with pytest.MonkeyPatch.context() as patch:
        ...
        patch.setattr(nm, 'fixed_value_mask', _mask)
        with pytest.raises(ValueError, match='frozen seed mass'):
            fi.fit_interior(2023, seed=seed, trade_electricity_pin=True)


def test_wedge_matches_free_active_closed_form() -> None:
    seed = _seed()
    commodities = list(seed.index)
    industries = list(seed.columns)
    irow, jcol = 1, 2
    ...
    row_t = seed.sum(axis=1) * 1.25
    col_t = seed.sum(axis=0) * 1.20  # deliberate wedge
    ...
    free = ~fixed_np
    # All axes active in this toy (full support, matching signs).
    r_a = np.ones(len(commodities), dtype=bool)
    c_a = np.ones(len(industries), dtype=bool)
    frozen_on_active_rows = float((seed_np * fixed_np * r_a[:, None]).sum())
    frozen_on_active_cols = float((seed_np * fixed_np * c_a[None, :]).sum())
    free_active_row_total = float(row_t.to_numpy()[r_a].sum()) - frozen_on_active_rows
    free_active_col_total = float(col_t.to_numpy()[c_a].sum()) - frozen_on_active_cols
    held_row_seed_nonfixed = float((seed_np * free * (~r_a)[:, None]).sum())
    held_col_seed_nonfixed = float((seed_np * free * (~c_a)[None, :]).sum())
    expected = free_active_row_total - (
        free_active_col_total + held_col_seed_nonfixed - held_row_seed_nonfixed
    )
    assert result.wedge_usd == pytest.approx(expected, abs=1.0)
    assert float(result.interior.iloc[irow, jcol]) == pytest.approx(
        float(seed.iloc[irow, jcol]), abs=1e-6
    )
