...
def derive_E_usa() -> pd.DataFrame:
    """Return E_usa (ghg × model-schema sectors), loaded from the flowsa FBS."""
...
def load_E_from_flowsa() -> pd.DataFrame:
    ...
    # CH4: use CH4_non_fossil when meta source is table 5_* or when in 2_1 and
    # sector starts with 1 or 562 or 2213 (biogenic sources per the EPA GHGI)
    ...
    if get_usa_config().use_cornerstone_2026_model_schema:
        ...
        # E_usa already has Cornerstone columns from map_fbs_sectors_to_model_schema
