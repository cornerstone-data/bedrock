...
from bedrock.utils.mapping.location import apply_county_FIPS, us_state_abbrev
...
def parse_statior(*, source, year, config, **_):
    ...
    for state in us_state_abbrev.keys():
        ...
        df = df.set_index('col')
        df2 = df.melt(
            ignore_index=False, value_name='FlowAmount', var_name='ActivityConsumedBy'
        )
        df2['ActivityProducedBy'] = df2.index
        if source == 'stateio_Make_Summary':
            # Adjust the index by removing the state: STATE.SECTOR
            df2['ActivityProducedBy'] = df2['ActivityProducedBy'].str.split(
                ".", expand=True
            )[1]
