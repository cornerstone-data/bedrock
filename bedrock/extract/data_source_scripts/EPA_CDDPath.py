...
def call_cddpath_model(*, resp, year, config, **_):
    ...
    if year == '2014':
        ...
        if os.path.isfile(source_data):
            ...
            log.error(
                f"{file} not found in external data directory. "
    ...
    df1 = (
        ...
        .melt(
            id_vars=["FlowName"], var_name="ActivityConsumedBy", value_name="FlowAmount"
        )
    )
...
def combine_cdd_path(*, resp, year, config, **_):
    ...
    if df_csv is None:
        ...
        df_csv = pd.read_csv(
            externaldatapath / file,
            header=0,
            names=['FlowName', 'ActivityProducedBy', 'FlowAmount'],
        )
    df_csv['pct'] = df_csv['FlowAmount'] / df_csv.groupby(
        ['FlowName']
...
def call_generation_by_source(file_dict):
    ...
    # melt
    df2 = df.melt(
        id_vars=["FlowName"], var_name="ActivityProducedBy", value_name="FlowAmount"
    )
    return df2
...
def cdd_processing(fba, source_dict):
    ...
    outputs.loc[outputs["ActivityConsumedBy"] == "Landfill", "Mixed CDD Processing"] = (
        landfill_from_mixed
    )
    ...
    outputs.loc[
        outputs['ActivityConsumedBy'] == 'Remanufacture', 'ActivityConsumedBy'
    ] = f'Remanufacture - {material}'
    outputs['Flowable'] = outputs['ActivityConsumedBy']
    outputs['FlowType'] = 'TECHNOSPHERE_FLOW'
    outputs.loc[outputs['Flowable'] == 'Landfill', ['Flowable', 'FlowType']] = [
        f"{material} waste",
        'WASTE_FLOW',
    ]
