...
def load_acs_variables(param='profile'):
    data = make_url_request(
        f'https://api.census.gov/data/2022/acs/acs5/{param}/variables.json'
    )
...
def DP_URL_helper(*, build_url, config, year, **_):
    ...
    for table, values in config['tables'].items():
        url = build_url.replace('__param__', values['param']).replace(
            '__group__', values['group']
        )
        urls.append(url)
...
def DP_call(*, resp, **_):
    ...
    df = pd.DataFrame(data=json_data[1 : len(json_data)], columns=json_data[0])
    return df
...
def DP_5yr_parse(*, df_list, config, year, **_):
    ...
    for df0 in df_list:
        ...
        year_dict = {
            k: v
            for k, v in table_dict['variables'].items()
            if year in parse_years(v.get('years', year))
        }
        description = {
            k: v['label']
            for k, v in load_acs_variables(table_dict['param']).items()
        ...
        df0 = (
            ...
            .assign(
                Description=lambda x: x[['code', 'Description']]
                .agg(': '.join, axis=1)
                .str.replace('!!', ', ')
            )
    ...
    # modify units
    df["Unit"] = (
        df['Unit'].str.split(' ').str[0].replace("#", "p").replace("%", "Percent")
    )
