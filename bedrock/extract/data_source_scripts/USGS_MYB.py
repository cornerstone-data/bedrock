...
def usgs_myb_year(years, current_year_str):
    ...
    if lower_year <= current_year <= upper_year:
        ...
        log.info(
            f"Your year is out of scope. Pick a year between "
...
def usgs_myb_url_helper(*, build_url, config, year, **_):
    ...
    url = (
        build_url.replace(
            '__FILENAME__',
            config.get('filename_replacement', {}).get(int(year), 'NULL'),
        )
        .replace('__YEAR__', year)
        .replace('__FORMAT__', config.get('file_format', {}).get(int(year), 'NULL'))
    )
...
def usgs_asbestos_call(*, resp, year, **_):
    ...
    """
    df_raw_data = pd.io.excel.read_excel(io.BytesIO(resp.content), sheet_name='T1')
    ...
    for df in df_list:
        for index, row in df.iterrows():
            ...
            if df.iloc[index]["Production"].strip() in row_to_use:
                ...
                dataframe = pd.concat(
                    [dataframe, pd.DataFrame.from_dict([data])], ignore_index=True
                )
    ...
    for df in df_list:
        for index, row in df.iterrows():
            ...
            if df.iloc[index]["Production"].strip() in row_to_use:
                ...
                col_name = usgs_myb_year(YEARS_COVERED['barite'], year)
                if (
                    str(df.iloc[index][col_name]) == "--"
                    or str(df.iloc[index][col_name]) == "(3)"
                ):
                    data["FlowAmount"] = str(0)
                else:
                    data["FlowAmount"] = str(df.iloc[index][col_name])
...
def usgs_bauxite_call(*, resp, year, **_):
    ...
    :return: pandas dataframe of original source data
    """
    df_raw_data_one = pd.io.excel.read_excel(io.BytesIO(resp.content), sheet_name='T1')
...
def usgs_beryllium_call(*, resp, year, **_):
    ...
    :return: pandas dataframe of original source data
    """
    df_raw_data = pd.io.excel.read_excel(io.BytesIO(resp.content), sheet_name='T4')

    df_raw_data_two = pd.io.excel.read_excel(io.BytesIO(resp.content), sheet_name='T1')
...
def usgs_boron_parse(*, df_list, source, year, **_):
    ...
    for df in df_list:
        for index, row in df.iterrows():
            ...
            if (
                df.iloc[index]["Production"].strip() == "B2O3 content"
                or df.iloc[index]["Production"].strip() == "Quantity"
            ):
                product = "production"
...
def usgs_clay_call(*, resp, year, **_):
    ...
    :return: pandas dataframe of original source data
    """
    df_raw_data_ball = pd.io.excel.read_excel(io.BytesIO(resp.content), sheet_name='T3')
    df_data_ball = pd.DataFrame(df_raw_data_ball.loc[19:19]).reindex()
    df_data_ball = df_data_ball.reset_index()
    del df_data_ball["index"]

    df_raw_data_bentonite = pd.io.excel.read_excel(
        io.BytesIO(resp.content), sheet_name='T4 '
    )
    ...
    df_raw_data_common = pd.io.excel.read_excel(
        io.BytesIO(resp.content), sheet_name='T5 '
    )
    ...
    df_raw_data_fire = pd.io.excel.read_excel(
        io.BytesIO(resp.content), sheet_name='T6 '
    )
    df_data_fire = pd.DataFrame(df_raw_data_fire.loc[12:12]).reindex()
    df_data_fire = df_data_fire.reset_index()
    del df_data_fire["index"]

    df_raw_data_fuller = pd.io.excel.read_excel(
        io.BytesIO(resp.content), sheet_name='T7 '
    )
    ...
    df_raw_data_kaolin = pd.io.excel.read_excel(
        io.BytesIO(resp.content), sheet_name='T8 '
    )
    ...
    df_raw_data_export = pd.io.excel.read_excel(
    ...
    df_raw_data_import = pd.io.excel.read_excel(
        io.BytesIO(resp.content), sheet_name='T14'
    )
        ...
        for index, row in df.iterrows():
            ...
            if str(df.iloc[index]["Production"]).strip() in row_to_use:
                ...
                if (
                    str(df.iloc[index][col_name]) == "--"
                    or str(df.iloc[index][col_name]) == "(3)"
                    or str(df.iloc[index][col_name]) == "(2)"
                ):
                    data["FlowAmount"] = str(0)
                else:
                    data["FlowAmount"] = str(df.iloc[index][col_name])
...
def usgs_cobalt_call(*, resp, year, **_):
    ...
    :return: pandas dataframe of original source data
    """
    df_raw_data = pd.io.excel.read_excel(io.BytesIO(resp.content), sheet_name='T8')
    df_raw_data_two = pd.io.excel.read_excel(io.BytesIO(resp.content), sheet_name='T1')
...
def usgs_fluorspar_call(*, resp, year, **_):
    ...
    if year in YEARS_COVERED['fluorspar_inports']:
        df_raw_data_two = pd.io.excel.read_excel(
            io.BytesIO(resp.content), sheet_name='T2'
        )
        df_raw_data_three = pd.io.excel.read_excel(
            io.BytesIO(resp.content), sheet_name='T7'
        )
        df_raw_data_four = pd.io.excel.read_excel(
            io.BytesIO(resp.content), sheet_name='T8'
        )
    ...
    for df in df_list:
        for index, row in df.iterrows():
            ...
            if str(df.iloc[index]["type"]).strip() == "data_two":
                ...
                str(df.iloc[index]["type"]).strip() == "Aluminum Fluoride"
                or str(df.iloc[index]["type"]).strip() == "Cryolite"
            ):
                prod = "imports"
...
def usgs_lead_call(*, resp, year, **_):
    ...
    :return: pandas dataframe of original source data
    """
    df = pd.read_excel(io.BytesIO(resp.content), sheet_name='T1', header=[3])
    df.columns = df.columns.astype(str).str.strip()
    df = df.rename(
        columns={
            df.columns[0]: 'Production',
            df.columns[1]: 'Units',
            year: 'FlowAmount',
        }
    )
    ...
    for df in df_list:
        for index, row in df.iterrows():
            if df.iloc[index]["Production"].strip() == "Exports":
                ...
                df.iloc[index]["Production"].strip() == "Secondary"
                or df.iloc[index]["Production"].strip() == "Primary"
            ):
...
def usgs_ma_call(*, resp, year, **_):
    ...
    :return: pandas dataframe of original source data
    """
    df_raw_data = pd.io.excel.read_excel(io.BytesIO(resp.content), sheet_name='T2')
    ...
    col_to_use = ["Product"]
    col_to_use.append(
        "quality_" + usgs_myb_year(YEARS_COVERED['manufacturedabrasive'], year)
    )
    for col in df_data.columns:
        if col not in col_to_use:
            del df_data[col]
    ...
    for df in df_list:
        for index, row in df.iterrows():
            ...
            if product in row_to_use:
                ...
                data["Unit"] = "Metric Tons"
                col_name = "quality_" + usgs_myb_year(
...
def usgs_nickel_call(*, resp, year, **_):
    ...
    :return: pandas dataframe of original source data
    """
    df_raw_data = pd.io.excel.read_excel(io.BytesIO(resp.content), sheet_name='T10')
    ...
    for df in df_list:
        ...
        for index, row in df.iterrows():
            ...
            if df.iloc[index]["Production"].strip() in row_to_use:
                ...
                if (
                    str(df.iloc[index][col_name]) == "--"
                    or str(df.iloc[index][col_name]) == "(4)"
                ):
                    data["FlowAmount"] = str(0)
                else:
                    data["FlowAmount"] = str(df.iloc[index][col_name])
...
def usgs_phosphate_call(*, resp, year, **_):
    ...
    df_raw_data_one.columns = df_raw_data_one.columns.astype(str).str.replace(".0", '')
...
def usgs_sgc_call(*, resp, year, **_):
    ...
    col_to_use = ["Production"]
    col_to_use.append(usgs_myb_year(YEARS_COVERED['sandgravelconstruction'], year))
    for col in df_data_1.columns:
        if col not in col_to_use:
            del df_data_1[col]
    ...
    for df in df_list:
        ...
        for index, row in df.iterrows():
            ...
            if df.iloc[index]["Production"].strip() in row_to_use:
                ...
                data["Year"] = str(year)
                data["Unit"] = "Thousand Metric Tons"
                col_name = usgs_myb_year(YEARS_COVERED['sandgravelconstruction'], year)
...
def usgs_sgi_call(*, resp, year, **_):
    ...
    col_to_use = ["Production"]
    col_to_use.append(usgs_myb_year(YEARS_COVERED['sandgravelindustrial'], year))
    for col in df_data_1.columns:
        if col not in col_to_use:
            del df_data_1[col]
            del df_data_2[col]
    ...
    for df in df_list:
        ...
        for index, row in df.iterrows():
            ...
            if df.iloc[index]["Production"].strip() in row_to_use:
                ...
                data["Year"] = str(year)
                data["Unit"] = "Thousand Metric Tons"
                col_name = usgs_myb_year(YEARS_COVERED['sandgravelindustrial'], year)
    ...
    for df in df_list:
        for index, row in df.iterrows():
            if df.iloc[index]["Production"].strip() == "Quantity":
                ...
                df.iloc[index]["Production"].strip() == "Imports for consumption, value"
            ):
                prod = "imports"
    ...
    for df in df_list:
        for index, row in df.iterrows():
            if df.iloc[index]["Production"].strip() == "Imports for consumption:2":
                ...
                df.iloc[index]["Production"].strip() == "Production, strontium minerals"
            ):
                product = "production"
            elif df.iloc[index]["Production"].strip() == "Exports:2":
                product = "exports"
    ...
    for df in df_list:
        for index, row in df.iterrows():
            if df.iloc[index]["Production"].strip() == "Imports for consumption":
                ...
                df.iloc[index]["Production"].strip() == "Production2"
                or df.iloc[index]["Production"].strip() == "Production"
            ):
                product = "production"
...
def usgs_zinc_call(*, resp, year, **_):
    ...
    df_raw_data_one = pd.io.excel.read_excel(io.BytesIO(resp.content), sheet_name='T9')
