...
def eia_mer_parse(*, df_list, config, **_):
    ...
    df = (
        df.assign(YYYYMM=lambda x: x['YYYYMM'].astype(str))
    ...
    df = (
        df.assign(Year=lambda x: x['YYYYMM'].str[:4])
    ...
    df['FlowType'] = np.where(
        df['Description'].isin(['T01.02']), 'ELEMENTARY_FLOW', 'TECHNOSPHERE_FLOW'
    )
...
def invert_heat_content(fba: FlowByActivity, **_) -> FlowByActivity:
    ...
    fba['FlowAmount'] = np.where(
        fba['Unit'].str.startswith('Million Btu'),
        (1 / fba['FlowAmount']) * 1e9,
        fba['FlowAmount'],
    )
    # convert btu to quadrilliion btu and invert
    fba['FlowAmount'] = np.where(
        fba['Unit'].str.startswith('Btu'),
    ...
    fba['Unit'] = np.where(
        fba['Unit'].str.endswith('Million Btu'),
        fba['Unit'].str.replace('Million Btu', 'Quadrillion Btu'),
        fba['Unit'],
    )
    fba['Unit'] = np.where(
        fba['Unit'].str.endswith('/Btu'),
        fba['Unit'].str.replace('/Btu', '/Quadrillion Btu'),
        fba['Unit'],
    )
    return fba
