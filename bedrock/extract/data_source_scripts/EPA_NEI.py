...
def epa_nei_url_helper(*, build_url, year, config, **_):
    ...
    url = (
        build_url.replace('__year__', year)
        .replace('__version__', version_dict[year])
        .replace(
            '__suffix__',
            config['url']
            .get('suffix', {})
            .get(year, config['url'].get('suffix', {}).get('base', '')),
        )
    )
    return [url]
...
def epa_nei_global_parse(*, df_list, source, year, config, **_):
    ...
    df = df.drop(columns=df.columns.difference(list(config['col_dict'][year].keys())))
...
def epa_nei_onroad_parse(*, df_list, source, year, config, **_):
    ...
    :return: df, parsed and partially formatted to flowbyactivity
        specifications
    """
    df = epa_nei_global_parse(df_list=df_list, source=source, year=year, config=config)
