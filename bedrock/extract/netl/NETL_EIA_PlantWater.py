...
def netl_eia_parse(*, source, year, **_):
    ...
    """
    # load the csv file
    DATA_FILE = f"NETL-EIA_powerplants_water_withdraw_consume_data_" f"{year}.csv"
    df_load = pd.read_csv(externaldatapath / DATA_FILE, index_col=0, low_memory=False)
    ...
    for c in cols_to_multiply:
        ...
        df = df.rename(
            columns={c: c.replace('Intensity Adjusted ', '').replace('/MWh', '')}
        )
    ...
    df2 = df2.rename(
        columns={
            'Total water discharge (million gallons) calc': 'Total water discharge (million gallons)'
        }
    )
    ...
    df3['Compartment'] = np.where(
        (df3['Compartment'] == '') & (df3['Water Source Name'].str.contains(ground)),
        "Ground",
        df3['Compartment'],
    )
    df3['Compartment'] = np.where(
        (df3['Compartment'] == '') & (df3['Water Source Name'].str.contains(surface)),
        "Surface",
        df3['Compartment'],
    )
    df3['Compartment'] = np.where(
    ...
    df3['Compartment'] = np.where(
        (df3['Compartment'] == '') & (df3['Water Source Name'].str.contains(reclaimed)),
        "Reclaimed",
        df3['Compartment'],
    )
    df3['Compartment'] = np.where(
        (df3['Compartment'] == '') & (df3['Water Source Name'].str.contains(storm)),
        "Stormwater",
        df3['Compartment'],
    )
    df3['Compartment'] = np.where(
    ...
    df4['ActivityConsumedBy'] = np.where(
        df4['FlowName'].isin(['Water Consumption (gal)', 'Water Withdrawal (gal)']),
        df4['Generator Primary Technology'],
        None,
    )
    df4['ActivityProducedBy'] = np.where(
        df4['ActivityConsumedBy'].isnull(), df4['Generator Primary Technology'], None
    )
    ...
    df4['FlowName'] = np.where(
        df4['FlowName'].str.contains('Water|water'),
    ...
    df4['Compartment'] = np.where(
        df4["FlowName"].str.contains('Consumption'), "Air", df4['Compartment']
    )

    df4['Class'] = np.where(
        df4['FlowName'].str.contains('Water|discharge'), "Water", "Energy"
    )
...
def clean_plantwater_fba(fba_df, **kwargs):
    ...
    sal_sub = fba_df[fba_df['FlowName'].str.contains('Saline')][
        ['FlowName', 'Flowable', 'Compartment', 'Context', 'FlowUUID']
    ].drop_duplicates()
    sal_sub = sal_sub.rename(
        columns={'FlowName': 'fn', 'Flowable': 'f', 'Context': 'ct', 'FlowUUID': 'fd'}
    )
    ...
    dfm['Flowable'] = np.where(
        dfm['FlowName'] == 'Brackish Water Withdrawal', dfm['f'], dfm['Flowable']
    )
    dfm['Context'] = np.where(
        dfm['FlowName'] == 'Brackish Water Withdrawal', dfm['ct'], dfm['Context']
    )
    dfm['FlowUUID'] = np.where(
        dfm['FlowName'] == 'Brackish Water Withdrawal', dfm['fd'], dfm['FlowUUID']
    )
    dfm['FlowName'] = np.where(
        dfm['FlowName'] == 'Brackish Water Withdrawal', dfm['fn'], dfm['FlowName']
    )
