...
def eia_aeo_url_helper(*, build_url, year, config, **_):
    ...
    list_seriesIDs = np.pad(
        list_seriesIDs, (0, rows * cols - len(list_seriesIDs)), mode='constant'
    )
...
def eia_aeo_call(*, resp, **_):
    ...
    df = (
        df_load.query("period==2020")
        .rename(
            columns={"seriesName": "Description", "value": "FlowAmount", "unit": "Unit"}
        )
        .drop(
            columns=[
                'history',
...
def eia_aeo_parse(*, df_list, year, **_):
    ...
    for index, row in df.iterrows():
        ...
        if len(name_array) == 4:
            ...
            if name_array[0] in ['Residential', 'Commercial', 'Transportation']:
