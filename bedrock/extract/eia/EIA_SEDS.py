...
def eia_seds_parse(
    *, df_list: List[pd.DataFrame], year: str, config: dict[str, Any], **_: Any
    ...
    fuels = units.query("SectorCode.isin(['TC', 'IC'])").reset_index(drop=True)
    ...
    # get sector names
    sectors = units.copy()
    sectors['ActivityConsumedBy'] = (
        units.query("Description.str.contains('consumed by')")
        .Description.str.split('consumed by the ', expand=True)[1]
        .str.strip()
    )
    sectors = (
        sectors[['SectorCode', 'ActivityConsumedBy']]
        .dropna()
    ...
    df = (
        df.rename(columns={year: "FlowAmount", "Fuel": "FlowName"})
    ...
    # hard code data
    df['Class'] = np.where(
        df['Unit'].str.contains('Btu') | df['Unit'].str.contains('watt'),
        'Energy',
        'Other',
    )
