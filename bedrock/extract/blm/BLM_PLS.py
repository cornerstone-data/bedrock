...
def split(
    ...
    row['one'] = row['one'].replace('Leases', ' Leases ').replace('  ', ' ').strip()

    flow_amount: str | float = ''
    num_in_state = False
    ...
    if (
        split_str_one[0] == 'North'
        or split_str_one[0] == 'South'
        or split_str_one[0] == 'West'
        or split_str_one[0] == 'New'
    ):
        ...
        if num_in_state:
            ...
            if len(split_str_one) == 1:
                ...
                if 'FFMC' in split_str_one:
                    ...
                    'Leases' in split_str_one[2]
                    or 'III' in split_str_one[2]
                    or 'Act' in split_str_one
                ):
                    if next_line:
                        column = 6
                    else:
...
def blm_pls_call(*, resp: Any, year: str, **_: Any) -> pd.DataFrame:
    ...
    if year == '2007':
        sub_headers = {
            ...
            'Coal Licenses': {'Exploration Licenses': [124], 'Licenses to Mine': [124]},
        ...
        sub_headers = {
            ...
            'Coal Licenses': {'Exploration Licenses': [137], 'Licenses To Mine': [137]},
        ...
        sub_headers = {
            ...
            'Coal Licenses': {'Exploration Licenses': [135], 'Licenses To Mine': [135]},
        ...
        for sub_header in sub_headers[header]:
            ...
            for page in pdf_pages:
                for index, row in page.iterrows():
                    ...
                    if (
                        copy
                        and split_row != sub_header
                        and split_row != header
                        and found_header
                    ):
                        ...
                        if not skip:
                            ...
                            if header in duplicate_headers:
                                ...
                                if row['page_no'] in competitive_page_numbers:
                                    ...
                                    flow_name.append(f'Noncompetitive {lists[1]}')
                            else:
                            ...
                            if 'Total' in row['one']:
                                ...
                                if page_shape == 1 and row['one'] == 'Total':
                                    next_line = True
                                elif (
                                    row_one_str.strip() == 'Total'
                                    or 'Leases' in row['one']
                                    or 'None' in row['one']
                                ):
