...
def census_cbp_parse(*, df_list, year, **_):
    ...
    df = pd.merge(
        df1,
        df2,
        on=["Location", "ActivityProducedBy", "Year", "Description", "FlowName"],
    )
    # Assign suppressed column
    df = df.assign(
        Suppressed=np.where(df.Note.isnull(), np.nan, df.Note),
        # FlowAmount = np.where(df.Note.isnull()),
        #                       0, df.FlowAmount)
    ).drop(columns='Note')
    # specify unit based on flowname, payroll in units of thousdand USD
    df = (
        df.assign(Unit=lambda x: np.where(x['FlowName'] == 'PAYANN', 'USD', 'p'))
        .assign(
        ...
        .assign(
            FlowName=lambda x: x['FlowName'].map(
                {
    ...
    df['Class'] = np.select(
        [
            df['FlowName'] == 'Number of employees',
