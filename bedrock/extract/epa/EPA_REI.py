...
def primary_factors_parse(*, df_list, year, **_):
    ...
    for df in df_list:
        ...
        if df['Description'][0] == 'primaryfactors':
            ...
            df = df.melt(
                id_vars=["Description", "ActivityProducedBy"],
                var_name="FlowName",
        ...
        # df for waste - sector consumed by
            ...
            df = (
                ...
                .melt(
                    id_vars=['Description', 'FlowName'],
                    var_name='ActivityConsumedBy',
                    value_name='FlowAmount',
                )
            )
            df = df[~df['FlowAmount'].astype(str).str.contains('- ')].reset_index(
                drop=True
            )
            ...
            df['FlowName'] = df['FlowName'].apply(lambda x: x.split('(', 1)[0])
            df["Class"] = "Other"
        ...
        # df for waste - sector produced by
            ...
            df['FlowAmount'] = df['FlowAmount'].str.replace(',', '').astype('float')
            df['Unit'] = 'MT'
            df['Unit'] = np.where(df['FlowName'].str.contains('\$'), 'USD', df['Unit'])
            df['FlowName'] = df['FlowName'].apply(lambda x: x.split('(', 1)[0])
            df["Class"] = "Other"
            df['FlowType'] = 'WASTE_FLOW'
        rei_list.append(df)
    ...
    df2['FlowName'] = np.where(df2['FlowName'] == 'Employment', 'Jobs', df2['FlowName'])
...
def rei_waste_national_cleanup(fbs: FlowBySector, **_) -> FlowBySector:
    ...
    """
    fbs = fbs.query('SectorConsumedBy not in ("F04000", "F05000")').reset_index(
        drop=True
    )
