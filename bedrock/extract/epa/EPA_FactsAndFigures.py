...
def ff_call(*, resp: Any, year: str, **_: Any) -> pd.DataFrame:
    ...
    for page_number in pages:
        ...
        if page_number == 6:
            ...
            # split column
            pg[['2000', '2005']] = pg['2000 2005'].str.split(' ', expand=True)
            ...
            pg.loc[16:21, "ActivityConsumedBy"] = pg["FlowName"].str.replace(
                "Food - ", ''
            )
            pg["ActivityConsumedBy"] = pg["ActivityConsumedBy"].str.title()
            pg['FlowName'] = pg['FlowName'].str.replace("( -).*", "", regex=True)
            # melt df and rename cols to standardize before merging with
            # additional tables
            pg = pg.melt(
                id_vars=["FlowName", "ActivityConsumedBy"],
                var_name="Year",
                value_name="FlowAmount",
            )
            pg["Description"] = (
                "Table 2. Materials Recycled, Composted and "
        ...
        if page_number in [8, 9]:
            ...
            if page_number == 9:
                ...
                pg["Description"] = (
                    "Table 4. Materials Landfilled in the " "Municipal Waste Stream"
                )
        # following code used for page 6, 9
        # drop nas and harcode metals and inorganic wastes back in
        pg["FlowName"] = np.where(
            pg["FlowName"].str.contains("Ferrous|Aluminum|Other Nonferrous"),
            'Metals, ' + pg["FlowName"],
            pg["FlowName"],
        )
        pg["FlowName"] = np.where(
            pg["FlowName"] == "Wastes",
            "Miscellaneous Inorganic " + pg["FlowName"],
            pg["FlowName"],
        )
        ...
        pg = pg[~pg["FlowName"].str.contains('Total')].reset_index(drop=True)
...
def ff_parse(*, df_list: list[pd.DataFrame], year: str, **_: Any) -> pd.DataFrame:
    ...
    df["FlowName"] = df["FlowName"].str.replace('[^a-zA-Z0-9, ]', '', regex=True)
