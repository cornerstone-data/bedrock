...
from bedrock.flowsa.naics import industry_spec_key, return_max_sector_level
...
def bls_qcew_parse(*, df_list, year, **_):
    ...
    float_cols = [
        ...
        if col not in ['area_fips', 'own_code', 'industry_code', 'year']
    ]
...
def clean_qcew(fba: FlowByActivity, **kwargs):
    ...
    totals = (
        fba.query('ActivityProducedBy.str.len() == 3')[
            ['Location', 'ActivityProducedBy', 'FlowAmount']
        ]
        .assign(ActivityProducedBy=lambda x: (x.ActivityProducedBy.str.slice(stop=2)))
        .groupby(['Location', 'ActivityProducedBy'])
        .agg('sum')
        .reset_index()
    ...
    fixed = (
        merged.assign(
            FlowAmount=merged.FlowAmount.mask(
                (merged.ActivityProducedBy.str.len() == 2) & (merged.FlowAmount == 0),
                merged.new_total,
            )
        )
...
def estimate_suppressed_qcew(fba: FlowByActivity) -> FlowByActivity:
    ...
    fba2 = (
        fba.assign(Unattributed=fba.FlowAmount.copy(), Attributed=0)
        .assign(descendants='')
        .replace({'ActivityProducedBy': {'31-33': '3X', '44-45': '4X', '48-49': '4Y'}})
    )
    ...
    fba3 = fba2.query(f'ActivityProducedBy.str.len() <= {max_level}').reset_index(
        drop=True
    )
    ...
    for level in range(max_level - 1, 1, -1):
        ...
        descendants = pd.DataFrame(
            ...
            .rename(
                columns={
                    'ActivityProducedBy': 'descendants',
    ...
    indexed = (
        fba3.assign(
            n2=fba3.ActivityProducedBy.str.slice(stop=2),
        ...
        .set_index(
            ['n2', 'n3', 'n4', 'n5', 'n6', 'location', 'category'],
    ...
    aggregated = (
        ...
        .replace({'ActivityProducedBy': {'3X': '31-33', '4X': '44-45', '4Y': '48-49'}})
        .aggregate_flowby()
    )
