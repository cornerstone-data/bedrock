...
from bedrock.flowsa.naics import industry_spec_key
...
def mlu_parse(*, df_list, source, year, **_):
    ...
    """
    # concat dataframes
    df = pd.concat(df_list, sort=False).rename(
        columns={
...
def attribute_transportation_sector_fees_to_target_sectors(fba, sector_col):
    ...
    df_fha2 = (
        df_fha.assign(
            count=lambda x: x.groupby('NAICS_2012_Code')['NAICS_2012_Code'].transform(
                'count'
            )
        )
...
def validate_land_attribution(fba, attributed_fba):
    ...
    :return:
    """
    fbsum = (fba[['group_id', 'group_total']].drop_duplicates())['group_total'].sum()
    attsum = attributed_fba['FlowAmount'].sum()
    percent_change = round(((attsum - fbsum) / fbsum) * 100, 3)
    if percent_change == 0:
        log.info(f"No change in {fba.full_name} FlowAmount after " "attribution.")
    else:
        log.warning(
            f"Percent change in {fba.full_name} after "
...
def allocate_usda_ers_mlu_land_in_urban_areas(
    ...
    vlog.info(
        'Assuming total land use from MECS and CBECS included '
    ...
    df_fha = attribute_transportation_sector_fees_to_target_sectors(fba, sector_col)
    ...
    if fba.config['geoscale'] == 'state':
        df_residential = df_residential.assign(
            rural_res=residential_land_area,
            total_area=df_residential['FlowAmount'].sum(),
            FlowAmount=lambda x: x['FlowAmount'] / x['total_area'] * x['rural_res'],
        )
    else:
    ...
    df_non_urban_transport_area = pd.concat(
        [df_residential, df_openspace, cbecs, mecs], sort=False, ignore_index=True
    )
    df_non_urban_transport_area = df_non_urban_transport_area[
        ['Location', 'Unit', 'FlowAmount']
    ]
    non_urban_transport_area_sum = (
        df_non_urban_transport_area.groupby(['Location', 'Unit'], as_index=False)
        .agg({'FlowAmount': "sum"})
        .rename(columns={'FlowAmount': 'NonTransport'})
    )
    # compare units
    ...
    air_rail_area_sum = (
        air_rail_area.groupby(['Location', 'Unit'], as_index=False)
        .agg({'FlowAmount': "sum"})
        .rename(columns={'FlowAmount': 'AirRail'})
    )
    ...
    allocated_urban_areas_df = (
        allocated_urban_areas_df.drop(
            columns=['group_id', 'group_total', 'rurl_res', 'total_area'],
            errors='ignore',
        )
        .reset_index(drop=True)
...
def allocate_usda_ers_mlu_land_in_rural_transportation_areas(
    ...
    allocated_rural_trans = (
        allocated_rural_trans.drop(columns=['group_id', 'group_total'])
        .reset_index(drop=True)
        .reset_index(names='group_id')
...
def allocate_usda_ers_mlu_other_land(fba: FlowByActivity, **_) -> FlowByActivity:
    ...
    # location = 00000, replace with rural res value
        ...
        fba2 = fba.assign(
            FlowAmount=np.where(
                fba['SectorConsumedBy'].isin(household), rural_res, fba['FlowAmount']
            )
        )
    else:
        log.error(
            'Geograpic level is not supported - function is written '
