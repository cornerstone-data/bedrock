...
from bedrock.flowsa.literature_values import (
...
def eia_cbecs_land_call(*, resp, url, **_):
    ...
    if "b5.xlsx" in url:
        ...
        df_rse = df_rse.melt(id_vars=["Name"], var_name="Location", value_name="Spread")
        df_data = df_data.melt(
            id_vars=["Name"], var_name="Location", value_name="FlowAmount"
        )
    if "b12.xlsx" in url:
        # skip rows and remove extra rows at end of dataframe
        ...
        df_rse = df_rse.melt(
            id_vars=["Description"], var_name="Name", value_name="Spread"
        )
        df_data = df_data.melt(
            id_vars=["Description"], var_name="Name", value_name="FlowAmount"
        )
    if "b14.xlsx" in url:
        # skip rows and remove extra rows at end of dataframe
...
def eia_cbecs_land_parse(*, df_list, year, **_):
    ...
    for dataframes in df_list:
        ...
        if "Location" not in list(dataframes):
            ...
            dataframes = dataframes.drop(
                dataframes[dataframes.ActivityConsumedBy == "Before 1920"].index
            )
            # rename location
            ...
            dataframes.loc[dataframes.Location == US_FIPS, "LocationSystem"] = (
                "FIPS_2010"
            )
    ...
    df['FlowName'] = df['FlowName'].apply(
        lambda x: x.replace('Total floorspace, All buildings', 'Total floorspace')
    )
...
def standardize_eia_cbecs_land_activity_names(df, column_to_standardize):
    ...
    df[column_to_standardize] = df.apply(
        lambda x: clean_str_and_capitalize(x[column_to_standardize]), axis=1
    )
...
def calculate_floorspace_based_on_number_of_floors(fba_load):
    ...
    vlog.info(
        'Calculate floorspace for mall and nonmall buildings '
    ...
    vlog.info(
        'Due to data suppression for floorspace by building '
            ...
            lambda x: (
                ...
                if 'One' in x
                    ...
                    if 'Two' in x
                        ...
                        3
                        if 'Three' in x
                        else (6 if 'Four' in x else (15 if 'Ten' in x else ""))
                    )
                )
            )
        )
    ...
    vlog.info(
        'Drop flows for "All Buildings" to avoid double '
...
def disaggregate_eia_cbecs_mercentile(df_load):
    ...
    df_merc = df_load[df_load['ActivityConsumedBy'].isin(['Mercantile'])].reset_index(
        drop=True
    )
    df_merc = df_merc[
        ['FlowAmount', 'Unit', 'Location', 'LocationSystem', 'Year', 'Description']
    ]
    ...
    # update flownames
    df_mall3['Flowable'] = df_mall3['Flowable'] + ', ' + df_mall3['Description']
...
def disaggregate_eia_cbecs_vacant_and_other(df_load):
    ...
    df_all = df_load[df_load['ActivityConsumedBy'].isin(['All buildings'])].reset_index(
        drop=True
    )
    ...
    df_vo = df_load[
        df_load['ActivityConsumedBy'].isin(['Vacant', 'Other'])
    ].reset_index(drop=True)
    df_vo = df_vo.assign(
        Denominator=df_vo.groupby(['Location', 'LocationSystem', 'Year'])[
            'FlowAmount'
        ].transform('sum')
    )
    ...
    df_vo2 = df_vo.merge(
        df_rem[['Unit', 'Location', 'LocationSystem', 'Remainder', 'Description']]
    )
...
def calculate_total_facility_land_area(df):
    ...
    vlog.info(
        'Modifying FlowAmounts - Assuming the floor space to ' 'land area ratio is 1:4'
    )
    df = df.assign(
        FlowAmount=(df['FlowAmount'] / floor_space_to_land_area_ratio)
        - df['FlowAmount']
    )
