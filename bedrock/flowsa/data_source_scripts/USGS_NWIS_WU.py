...
def usgs_call(*, resp, url, **_):
    ...
    if "County" in url:
        ...
        # parse data frames
        df_usgs = df_usgs.rename(
            columns={
                df_usgs.columns[1]: "Description",
...
def usgs_parse(*, df_list, year, **_):
    ...
    df_sc = pd.melt(
        df_sc,
        id_vars=["geo", "state_cd", "state_name", "county_cd", "county_nm", "year"],
    ...
    df.loc[:, 'FlowName'] = np.where(
        df.Description.str.contains("fresh"),
        "fresh",
        np.where(
            df.Description.str.contains("saline"),
            "saline",
    ...
    # create flow name column
    df.loc[:, 'Compartment'] = np.where(
        df.Description.str.contains("ground|Ground"), "ground", "total"
    )
    df.loc[:, 'Compartment'] = np.where(
        df.Description.str.contains("surface|Surface"), "surface", df['Compartment']
    )
    # consumptive water use is reported nationally for thermoelectric
    # and irrigation beginning in 2015
    df.loc[:, 'Compartment'] = np.where(
        df.Description.str.contains("consumptive"), "air", df['Compartment']
    )
    ...
    df.loc[
        df['ActivityConsumedBy'].isin(['Public Supply', 'Public supply']),
        'DataReliability',
    ] = 2
    df.loc[
    ...
    df['FlowType'] = np.where(
        df["Description"].str.contains('wastewater'), "WASTE_FLOW", "ELEMENTARY_FLOW"
    )
    # is really a "TECHNOSPHERE_FLOW"
    df['FlowType'] = np.where(
        df["Description"].str.contains('deliveries'), "ELEMENTARY_FLOW", df['FlowType']
    )
...
def standardize_usgs_nwis_names(flowbyactivity_df):
    ...
    for f in fba_activity_fields:
        ...
        flowbyactivity_df.loc[flowbyactivity_df[f] == 'Irrigation Total', f] = (
            'Irrigation'
        )
        flowbyactivity_df.loc[
            flowbyactivity_df[f] == 'Total Thermoelectric Power', f
        ] = 'Thermoelectric Power'
...
def usgs_fba_data_cleanup(fba: FlowByActivity) -> FlowByActivity:
    ...
    vlog.info(
        'If states do not distinguish between golf and crop '
    ...
    vlog.info(
        'Drop rows where the FlowName is total to prevent '
...
def calculate_net_public_supply(df_load: FlowByActivity):
    ...
    df1 = df_load[
        (df_load[fba_activity_fields[0]] == 'Public Supply')
    ...
    df1_sub = df1[
        ~df1[fba_activity_fields[1]].isin(
            [
                'Industrial',
                'Thermoelectric Power',
                'Thermoelectric Power Closed-loop cooling',
    ...
    vlog.info(
        'Dropping county level public supply withdrawals '
        'because will end up with negative values due to '
        'instances of water deliveries coming from surrounding '
        'counties'
    )
    df1_sub = df1_sub[df1_sub['Location'].apply(lambda x: x[2:6] == '000')].reset_index(
        drop=True
    )
    ...
    df_wm = df_wm.rename(
        columns={"FlowAmount_x": "FlowAmount", "FlowAmount_y": "FlowTotal"}
    )
    # compare units
    compare_df_units(df_wm, df_d)
    # merge the deliveries to domestic
    df_w_modified = pd.merge(
        df_wm,
    ...
    df_w_modified = df_w_modified.rename(
        columns={"FlowAmount_x": "FlowAmount", "FlowAmount_y": "DomesticDeliveries"}
    )
    ...
    df_d_modified = pd.merge(
        df_d_modified,
        net_ps[['FlowName', 'Compartment', 'Location', 'FlowRatio']],
        how='left',
        on='Location',
    )
    # DC has flowratio of 0, so replace (net public supply = 0)
    df_d_modified['FlowRatio'] = np.where(
        (df_d_modified['Location'] == '11000') & (df_d_modified['FlowRatio'].isna()),
        0.5,
        df_d_modified['FlowRatio'],
    )
    ...
    modified_ps = pd.concat([df2, df_d_modified, net_ps, df_us], ignore_index=True)
...
def check_golf_and_crop_irrigation_totals(df_load: FlowByActivity):
    ...
    df_m3.loc[df_m3['ActivityProducedBy'] == 'Irrigation', 'ActivityProducedBy'] = (
        'Irrigation Crop'
    )
    df_m3.loc[df_m3['ActivityConsumedBy'] == 'Irrigation', 'ActivityConsumedBy'] = (
        'Irrigation Crop'
    )
    df_m3['Description'] = (
        df_m3['Description']
        .str.replace('Irrigation, Total', 'Irrigation, Crop')
        .str.replace('withdrawals', 'withdrawals for crops')
        .str.replace('use', 'use for crops')
    )
    df_m3 = df_m3.drop(
        columns=[
            'Golf_Amount',
    ...
    if len(df_m3) != 0:
        ...
        df_w_missing_crop2 = df_w_missing_crop.aggregate_flowby(
            retain_zeros=True, columns_to_group_by=group_cols, aggregate_ratios=True
        )

        # validate results - the differences should all be 0
        df_check = subset_and_merge_irrigation_types(df_w_missing_crop2)
        df_check = df_check[df_check['Location'] != US_FIPS].reset_index(drop=True)
        df_check['Diff'] = df_check['Diff'].apply(lambda x: round(x, 2))
        df_check2 = df_check[df_check['Diff'] != 0]
        if len(df_check2) > 0:
            vlog.info(
                'The golf and crop irrigation do not add up to ' 'total irrigation.'
            )
        else:
            vlog.info('The golf and crop irrigation add up to total ' 'irrigation.')
...
def subset_and_merge_irrigation_types(df: FlowByActivity):
    ...
    df_i = df[
        (df[fba_activity_fields[0]] == 'Irrigation')
        | (df[fba_activity_fields[1]] == 'Irrigation')
    ]
    df_g = df[
        (df[fba_activity_fields[0]] == 'Irrigation Golf Courses')
        | (df[fba_activity_fields[1]] == 'Irrigation Golf Courses')
    ]
    df_c = df[
        (df[fba_activity_fields[0]] == 'Irrigation Crop')
