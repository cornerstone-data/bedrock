...
def epa_state_ghgi_call(*, resp, config, **_):
    ...
    with ZipFile(io.BytesIO(resp.content)) as z:
        df = pd.read_excel(z.open(config['file']), sheet_name=config['sheet'])
    return df
...
def epa_state_ghgi_parse(*, df_list, source, year, config, **_):
    ...
    df = (
        ...
        .rename(columns={'geo_ref': 'State', 'ghg': 'FlowName'})
        .assign(
            ActivityProducedBy=lambda x: x[activity_cols].apply(
                lambda row: " - ".join(row.dropna().drop_duplicates().astype(str)),
                axis=1,
            )
        )
...
def tag_biogenic_activities(fba, source_dict, **_):
    ...
    fba.loc[fba['ActivityProducedBy'].isin(a_list), 'FlowName'] = (
        fba['FlowName'] + ' - biogenic'
    )
...
def allocate_flows_by_fuel(fba: FlowByActivity, **_) -> FlowByActivity:
    ...
    fuels = {'Natural Gas': 'Natural Gas', 'Coal': 'Coal', 'Fuel Oil': 'Petroleum'}
    ...
    ratios = (
        ...
        .pivot_table(columns='FlowName', index='Fuel', values='FlowAmount')
    ...
    fba1 = pd.concat(
        ...
        Fuel=lambda x: x['ActivityProducedBy'].str.rsplit(' - ', n=1, expand=True)[1]
    )

    # Derive state CH4 and N2O emissions by fuel type using fuel specific ratios
    fba2 = (
        fba1.query('FlowName == "CO2"')
        .assign(
            Fuel=lambda x: x['Fuel'].replace(dict((v, k) for k, v in fuels.items()))
        )
        .merge(ratios.reset_index())
        .assign(CH4=lambda x: x['CH4'] * x['FlowAmount'])
        .assign(N2O=lambda x: x['N2O'] * x['FlowAmount'])
        .melt(
            id_vars=['Location', 'Fuel'], value_vars=['CH4', 'N2O'], var_name='FlowName'
        )
        .pivot_table(columns='Fuel', index=['Location', 'FlowName'], values='value')
    )
    ...
    if round(fba3.FlowAmount.sum(), 6) != round(fba.FlowAmount.sum(), 6):
        log.warning(
            'Error: totals do not match when splitting CH4 and N2O by ' 'fuel type'
        )
...
def allocate_industrial_combustion(fba: FlowByActivity, **_) -> FlowByActivity:
    ...
    """
    from bedrock.flowsa.data_source_scripts.EPA_GHGI import (
