...
def filter_by_geoscale(df, geoscale):
    ...
    if len(df) == 0:
        raise FBSMethodConstructionError(
            message="No flows found in the flow dataset at " f"the {geoscale} scale"
        )
    else:
        return df
...
def assign_fips_location_system(df, year_of_data):
    ...
    if year_of_data >= 2015:
        ...
        log.warning(
            f"Missing FIPS codes from crosswalk for {year_of_data}. "
...
def collapse_fbs_sectors(fbs):
    ...
    if fbs['SectorProducedBy'].isnull().all():
        ...
        fbs.loc[fbs["FlowType"] == 'TECHNOSPHERE_FLOW', 'Sector'] = fbs[
            "SectorConsumedBy"
        ]
        fbs.loc[fbs["FlowType"] == 'WASTE_FLOW', 'Sector'] = fbs["SectorProducedBy"]
        fbs.loc[
            (fbs["FlowType"] == 'WASTE_FLOW') & (fbs['SectorProducedBy'].isnull()),
            'Sector',
        ] = fbs["SectorConsumedBy"]
        fbs.loc[
            (fbs["FlowType"] == 'ELEMENTARY_FLOW') & (fbs['SectorProducedBy'].isnull()),
            'Sector',
        ] = fbs["SectorConsumedBy"]
        fbs.loc[
            (fbs["FlowType"] == 'ELEMENTARY_FLOW') & (fbs['SectorConsumedBy'].isnull()),
            'Sector',
    ...
    fbs_collapsed = fbs_collapsed.sort_values(
        ['Sector', 'Flowable', 'Context', 'Location']
...
def load_fba_w_standardized_units(datasource, year, **kwargs):
    ...
    if 'allocation_map_to_flow_list' in kwargs:
        if kwargs['allocation_map_to_flow_list']:
            ...
            fba = clean_df(fba, flow_by_activity_fields, drop_description=False)
            fba, mapping_files = map_fbs_flows(
                fba, datasource, kwargs, keep_fba_columns=True, keep_unmapped_rows=True
            )
        else:
            # ensure df loaded correctly/has correct dtypes
