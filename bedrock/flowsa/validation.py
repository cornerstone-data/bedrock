...
def calculate_flowamount_diff_between_dfs(dfa_load, dfb_load):
    ...
    dfa = dfa_load.drop(drop_cols, axis=1, errors='ignore').rename(
        columns={'FlowAmount': 'FlowAmount_Original'}
    )
    dfb = dfb_load.drop(drop_cols, axis=1, errors='ignore').rename(
        columns={'FlowAmount': 'FlowAmount_Modified'}
    )
    ...
    if len(dfn) > 0:
        ...
        vlog.info(
            'Negative FlowAmounts in new dataframe: ' '\n {}'.format(dfn.to_string())
        )
    ...
    dfagg['Percent_Increase'] = (
        dfagg['FlowAmount_Difference'] / dfagg['FlowAmount_Original']
    ) * 100
    ...
    if len(dfagg2) == 0:
        ...
        dfagg3 = dfagg.drop(
            columns=[
                'ActivityProducedBy',
                'ActivityConsumedBy',
                'FlowAmount_Difference',
                'Percent_Increase',
            ]
        )
        dfagg4 = dfagg3.groupby(
            flowcols + ['Unit', 'geoscale'], dropna=False, as_index=False
        ).agg({'FlowAmount_Original': "sum", 'FlowAmount_Modified': "sum"})
        # column calculating difference
        dfagg4['FlowAmount_Difference'] = (
            dfagg4['FlowAmount_Modified'] - dfagg4['FlowAmount_Original']
        )
        dfagg4['Percent_Increase'] = (
            dfagg4['FlowAmount_Difference'] / dfagg4['FlowAmount_Original']
        ) * 100
        # drop rows where difference = 0
        dfagg5 = dfagg4[dfagg4['FlowAmount_Difference'] != 0].reset_index(drop=True)
        vlog.info(
            'Total FlowAmount differences between dataframes: '
            '\n {}'.format(
                dfagg5.to_string(),
            )
        )
        ...
        vlog.info(
            'Total FlowAmount differences by Activity Columns: '
            '\n {}'.format(
                dfagg2.to_string(),
            )
        )
...
def compare_summation_at_sector_lengths_between_two_dfs(df1, df2):
    ...
    agg_cols = [
        e for e in agg_cols if e not in [f'{col}ProducedBy', f'{col}ConsumedBy']
    ]
    ...
    for df in [df1, df2]:
        ...
        for s in ['Produced', 'Consumed']:
            df = (
                df.merge(cw, how='left', left_on=f'{col}{s}By', right_on='Sector')
                .drop(columns=['Sector'])
                .rename(columns={'SectorLength': f'Sector{s}ByLength'})
            )
            df[f'Sector{s}ByLength'] = df[f'Sector{s}ByLength'].fillna(0)
        # sum flowamounts by sector length
        dfsum = (
            df.groupby(agg_cols, dropna=False).agg({'FlowAmount': 'sum'}).reset_index()
        )
    ...
    dfm2 = dfm[
        ~((dfm['SectorProducedByLength'] == 0) & (dfm['SectorConsumedByLength'] == 0))
    ]
    # sort df
    dfm2 = dfm2.sort_values(
        ['Location', 'SectorProducedByLength', 'SectorConsumedByLength']
    ...
    if len(dfm3) > 0:
        log.info(
            'See validation log for cases where the second dataframe '
            'has flow amounts greater than the first dataframe at the '
            'same location/sector lengths.'
        )
        vlog.info(
            'The second dataframe has flow amounts greater than '
            'the first dataframe at the same sector lengths: '
            '\n {}'.format(dfm3.to_string())
        )
    else:
        vlog.info(
            'The second dataframe does not have flow amounts '
...
def compare_FBA_results(
    ...
    merge_cols = [
        c
        for c in df2.select_dtypes(include=['object', 'int']).columns
    ...
    df_m = pd.DataFrame(
        pd.merge(
            df1[merge_cols + ['FlowAmount_fba1']],
            df2[merge_cols + ['FlowAmount_fba2']],
            how='outer',
        )
    )
    df_m = df_m.assign(
        FlowAmount_diff=df_m['FlowAmount_fba2'].fillna(0)
        - df_m['FlowAmount_fba1'].fillna(0)
    )
    df_m = df_m.assign(
...
def compare_FBS(df1_load, df2_load, ignore_metasources=False):
    ...
    df1_sub = (
        df1_sub.groupby(merge_cols, dropna=False)
        .agg({'FlowAmount_fbs1': 'sum'})
        .reset_index()
    )
    df2_sub = (
        df2_sub.groupby(merge_cols, dropna=False)
        .agg({'FlowAmount_fbs2': 'sum'})
        .reset_index()
    )
    ...
    df_m = df_m.assign(
        FlowAmount_diff=df_m['FlowAmount_fbs2'].fillna(0)
        - df_m['FlowAmount_fbs1'].fillna(0)
    )
    df_m = df_m.assign(
...
def compare_single_FBS_against_remote(m, outdir=diffpath, run_single=False):
    ...
    if not downloaded:
        if run_single:
            ...
            FlowBySector.generateFlowBySector(method=m, download_sources_ok=True)
        else:
    ...
    df = compare_FBS_results(m, m, ignore_metasources=True, compare_to_remote=True)
    df.rename(
        columns={
            'FlowAmount_fbs1': 'FlowAmount_remote',
...
def compare_single_FBA_against_remote(source, year, outdir=diffpath, run_single=False):
    ...
    df.rename(
        columns={
            'FlowAmount_fba1': 'FlowAmount_remote',
...
def compare_national_state_fbs(
    ...
    # and aggregate
    national_agg = national.sector_aggregation(industry_spec=state_target)[
        subset_fields
    ].aggregate_flowby()

    # attribute state data to national location, subset df,
    # and aggregate
    state_agg = state.convert_fips_to_geoscale(target_geoscale='national')[
        subset_fields
    ].aggregate_flowby()

    # compare FBS results
    sectors = pd.DataFrame(
        national_agg.merge(state_agg, how='outer', on=groupby_fields)
        .rename(columns={'FlowAmount_x': 'national', 'FlowAmount_y': 'state'})
        .drop(columns='Location')
    )
    sectors['comp'] = sectors['state'].fillna(0) / sectors['national']

    flows = (
        sectors.groupby('Flowable')
        .agg({'state': 'sum', 'national': 'sum'})
        .reset_index()
    )
...
def compare_geographic_totals(
    ...
    nat = (
        df_load[df_load['Location'] == US_FIPS]
        .reset_index(drop=True)
        .rename(columns={'FlowAmount': 'FlowAmount_nat'})
    )
    ...
    if len(nat) != 0:
        ...
        if '/' in nat['Unit'][0]:
            log.info(
                f"Skipping geoscale comparison because {nat['Unit'][0]} " f"is a rate."
            )
        else:
            # subset national level data by activity set names
            nat = nat[
                (nat[fba_activity_fields[0]].isin(activity_names))
            ...
            df_m_sub = df_m[
                (df_m['Percent_Increase'] > 1) | (df_m['Percent_Increase'].isna())
            ].reset_index(drop=True)

            subnational_geoscale = subnational_geoscale or attr['allocation_from_scale']
            if len(df_m_sub) == 0:
                vlog.info(
                    f'No data loss greater than 1% between national '
                    f'level data and {subnational_geoscale} subset'
                )
            else:
                vlog.info(
                    f'There are data differences between published national '
...
def rename_column_values_for_comparison(df, sourcename):
    ...
    if sourcename == 'USGS_NWIS_WU':
        ...
        df['Context'] = np.where(
            df['Context'].str.contains('resource/water/'),
            'resource/water',
            df['Context'],
        )
...
def calculate_industry_coefficients(fbs_load, year, region, io_level, impacts=False):
    ...
    if impacts:
        ...
        try:
            ...
            groupby_cols = ['Location', 'Sector', 'Indicator', 'Indicator unit']
    ...
    if inventory:
        ...
        groupby_cols = ['Location', 'Sector', 'Flowable', 'Context', 'Unit']
        sort_by_cols = ['Context', 'Flowable', 'Sector', 'Location']
    ...
    fbs_summary = fbs_summary.merge(
        bea.rename(columns={'BEA': 'Sector'}), how='left', on=['Sector', 'Location']
    )
    fbs_summary['Coefficient'] = fbs_summary['FlowAmount'] / fbs_summary['Output']
