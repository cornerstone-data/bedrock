...
from bedrock.flowsa.common import load_crosswalk, load_yaml_dict
...
from bedrock.flowsa.settings import datapath, plotoutputpath
...
def addSectorNames(df, BEA=False, mappingfile=None):
    ...
    if BEA:
        cw = pd.read_csv(
            'https://raw.githubusercontent.com/USEPA/useeior'
        ...
        cw['SectorName'] = np.where(
            cw['SectorName'].str.len() == 50,
            cw['SectorName'] + '...)',
            cw['SectorName'],
        )
    else:
        if mappingfile is not None:
            ...
            cw['SectorName'] = (
                cw['NAICS_2012_Code'].map(str) + ' (' + cw['NAICS_2012_Name'] + ')'
            )
    ...
    for s in sector_cols:
        df = df.merge(
            cw[['Sector', 'SectorName']], left_on=s, right_on='Sector', how='left'
        ).rename(columns={'SectorName': f'{s}Name'})
        if s != 'Sector':
...
def FBSscatterplot(
    ...
    if industry_spec is not None:
        ...
        # sector_aggregation()
        df = df.rename(columns={'Sector': 'SectorProducedBy'}).assign(
            SectorConsumedBy=np.nan
        )
        df = (
            df.sector_aggregation()
    ...
    df2 = df.groupby(['methodname', 'Sector', 'Unit'], as_index=False).agg(
        {"FlowAmount": sum}
    )
    ...
    if plottype == 'facet_graph':
        ...
        g = sns.FacetGrid(
            df2, col="methodname", sharex=False, aspect=1.5, margin_titles=False
        )
        ...
        g = sns.boxplot(data=df2, x="FlowAmount", y=y_axis, color="gray")
        g.set(xlabel=axis_title, ylabel="")
...
def stackedBarChart(
    ...
    # wrap the sector col
    df2[sector_variable] = df2[sector_variable].apply(lambda x: customwrap(x, width=12))
    ...
    if subplot is None:
        ...
        for row in range(1, rows + 1):
            for col in range(1, cols + 1):
                df3 = pd.DataFrame(
                    df2[df2[subplot] == plot_list[s]].reset_index(drop=True)
                )
...
def convert_units_for_graphics(df):
    ...
    df['FlowAmount'] = np.where(
        (df["Class"] == 'Other') & (df['Unit'] == 'kg'),
        df['FlowAmount'] / (10**9),
        df['FlowAmount'],
    )
    df['Unit'] = np.where(
        (df["Class"] == 'Other') & (df['Unit'] == 'kg'), "MMT", df['Unit']
    )

    # convert kg to million metric tons for class chemicals
    df['FlowAmount'] = np.where(
        (df["Class"] == 'Chemicals') & (df['Unit'] == 'kg'),
        df['FlowAmount'] / (10**9),
        df['FlowAmount'],
    )
    df['Unit'] = np.where(
        (df["Class"] == 'Chemicals') & (df['Unit'] == 'kg'), "MMT", df['Unit']
    )

    # convert people to thousand
    df['FlowAmount'] = np.where(
        df['Unit'] == 'p', df['FlowAmount'] / (1000), df['FlowAmount']
    )
