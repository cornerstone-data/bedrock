...
def assign_technological_correlation(mapping):
    ...
    mapping = mapping.assign(
        ...
        .astype(str)
        .apply(lambda x: tech_dict.get(x))
    )
...
def convert_units_to_annual(df):
    ...
    df['FlowAmount'] = np.where(
        df['Unit'].str.contains('/d'), df['FlowAmount'] * 365, df['FlowAmount']
    )
...
def map_flows(
    ...
    **kwargs,
    ...
    # else, rename
        ...
        fba = fba.rename(columns={'FlowName': 'Flowable', 'Compartment': 'Context'})
    ...
    if mapped_df is None or len(mapped_df) == 0:
        ...
        log.warning(
            "Error in flow mapping, flows not mapped, returning FBA "
...
def map_fbs_flows(fbs, from_fba_source, v, **kwargs):
    ...
    fbs_mapped = map_flows(fbs, mapping_files, flow_type, ignore_source_name, **kwargs)
...
def map_to_BEA_sectors(fbs_load, region, io_level, output_year, bea_year=2012):
    ...
    naics_year = fbs_load['SectorSourceName'][0].lower().split("_", 1)[1].split("_", 1)[0]

    # Prepare NAICS:BEA mapping file
    mapping = load_crosswalk(f'NAICS_to_BEA_Crosswalk_{bea_year}').rename(
        columns={mapping_col: 'BEA', f'NAICS_{naics_year}_Code': 'Sector'}
    )
    mapping = (
        mapping.drop(columns=mapping.columns.difference(['Sector', 'BEA']))
    ...
    if set(['SectorProducedBy', 'SectorConsumedBy']).issubset(fbs_load.columns):
        ...
        for col in ['SectorProducedBy', 'SectorConsumedBy']:
            ...
            fbs = fbs.drop(
                columns=[col, 'BEA_y', 'Allocation', 'Sector', 'Sector_y']
    ...
    if (abs(1 - (sum(fbs['FlowAmount']) / sum(fbs_load['FlowAmount'])))) > 0.005:
...
def get_BEA_industry_output(region, io_level, output_year, bea_year=2012):
    ...
    if region == 'national' and io_level == 'summary':
        bea_mapping = (
            load_crosswalk(f'NAICS_to_BEA_Crosswalk_{bea_year}')[
                [f'BEA_{bea_year}_Detail_Code', f'BEA_{bea_year}_Summary_Code']
            ]
            .drop_duplicates()
            .rename(columns={f'BEA_{bea_year}_Detail_Code': 'BEA'})
        )
        bea = (
            bea.merge(bea_mapping, how='left', on='BEA')
            .drop(columns=['BEA'])
...
def map_to_material_crosswalk(df, source, source_attr):
    ...
    if mapped_df is None or len(mapped_df) == 0:
        # return the original df but with columns renamed so
        # can continue working on the FBS
        log.warning("Error in mapping, flows not mapped to material " "crosswalk")
        mapped_df = df.copy()
...
def append_material_code(df, v, attr):
    ...
    if attr.get('material') is not None:
        ...
        for s in ['SectorProducedBy', 'SectorConsumedBy']:
            df[s] = np.where((df[s] is not None) and (df[s] != ''), df[s] + abbr, df[s])
    else:
        # add materials
        df = df.merge(mapping_file, left_on='Flowable', right_on='Material')
        for s in ['SectorProducedBy', 'SectorConsumedBy']:
            df[s] = np.where(
                (df[s] is not None) and (df[s] != ''), df[s] + df['Abbr'], df[s]
            )
