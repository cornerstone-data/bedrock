...
class FlowsaLoader(yaml.SafeLoader):
    ...
    def from_index(loader: 'FlowsaLoader', file: str, node: yaml.ScalarNode) -> list:
        ...
        with open(file, 'r', encoding='utf-8-sig', newline='') as f:
            ...
            return [row['name'] for row in index if row['activity_set'] == activity_set]
...
def load(stream: IO, external_path: str = None) -> dict:
    ...
    if external_path:
        ...
        loader.external_paths_to_search.append(f'{external_path}flowbysectormethods/')
        loader.external_paths_to_search.append(
            f'{external_path}flowbysectoractivitysets/'
        )
        loader.external_paths_to_search.append(path.dirname(external_path))
