...
def _extract_config_name(
    config_summary_map: dict[str, object], label: str
) -> str:
...
def merge_sheets(
    ...
    *,
    ...
    if include_pinned_useeio_baseline and pinned_baseline_d is not None:
        merged_d[PINNED_USEEIO_BASELINE_COLUMN] = (
            pinned_baseline_d.reindex(merged_d["sector"]).values
        )
    ...
    if include_pinned_useeio_baseline and pinned_baseline_n is not None:
        merged_n[PINNED_USEEIO_BASELINE_COLUMN] = (
            pinned_baseline_n.reindex(merged_n["sector"]).values
        )
    for config_name in config_names:
    ...
    if include_pinned_useeio_baseline:
        ...
        baseline_pin_map = {
            k: v
            for k, v in first_cfg_map.items()
            if k in _USEEIO_BASELINE_PIN_FIELDS
        }
    ...
    for (sheet_id, label), config_name in zip(
        sheet_inputs_in_order, config_names
    ):
        df_src = load_tabs_optional(
            sheet_id, [totals_source_tab], refresh=refresh
        )[totals_source_tab]
        if df_src is None:
            totals_skipped_reason = (
                f"tab {totals_source_tab!r} not found in Sheet '{label}' "
                f"({sheet_id})"
            )
            break
        if any_useeio:
