...
year scaling, and the Step 5 GRAS kernel.
...
allowed). Omitting ``sign_flex`` in that wrapper would sign-lock the
whole SUT. The wrapper ``engine(free, residual, masks)`` is not this
kernel and is not implemented yet.
