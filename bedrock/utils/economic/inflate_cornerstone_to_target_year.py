...
from bedrock.transform.iot.derived_price_index import prepare_formatted_bea_price_index
...
def get_cornerstone_industry_price_ratio(
    ...
    """Industry CPI ratio reindexed to cornerstone industry codes."""
    pi = prepare_formatted_bea_price_index()
    num_col = str(original_year)
    den_col = str(target_year)
    if num_col not in pi.columns or den_col not in pi.columns:
        raise ValueError(
            f"Price-index years unavailable: {original_year}, {target_year}"
        )

    bea_ratio = (
        (pi[num_col].astype(float) / pi[den_col].astype(float))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(1.0)
    )
    mapping = load_bea_v2017_industry_to_cornerstone_industry()

    ratio_parts: dict[str, list[float]] = {}
    for bea_code, corner_codes in mapping.items():
        val = float(bea_ratio.get(bea_code, 1.0))
        for code in corner_codes:
            ratio_parts.setdefault(code, []).append(val)

    ratio = pd.Series(
        {
            code: float(np.mean(ratio_parts.get(code, [1.0])))
            for code in CORNERSTONE_INDUSTRIES
        }
    )
    ratio.index.name = "sector"
    return ratio
