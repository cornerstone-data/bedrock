...
def load_bea_v2017_summary_to_ceda_v7() -> dict[
    BEA_2017_COMMODITY_SUMMARY_CODE, list[CEDA_V7_SECTOR]
]:
...
def load_bea_v2017_summary_to_cornerstone() -> dict[
    BEA_2017_COMMODITY_SUMMARY_CODE, list[str]
]:
...
def get_bea_v2017_summary_to_cornerstone_corresp_df() -> pd.DataFrame:
    ...
    corresp_df = create_correspondence_matrix(summary_to_cornerstone).reindex(
        index=COMMODITIES, fill_value=0
    )
