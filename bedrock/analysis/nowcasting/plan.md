...
✅ **Status, 2026-08-27: built and wired.** `derive_initial_U_intermediate` runs, and as of the
seed-wiring branch it starts from `composed_seed` rather than the 2017 benchmark alone. The level was
never Step 3's to estimate — `GO − VAPRO` supplies it — so what changed is the **shape**, which had
been 2017's in every year.
