...
**Phase 2 — the crosswalk.** ✅ **Built** as
`Sector_Crosswalk_BEA_NIPA_Inventories.csv`, generated by
[`scripts/write_inventories_trade_crosswalk.py`](../../../scripts/write_inventories_trade_crosswalk.py)
— 29 trade lines, 858 rows, 256 distinct commodities.

**The concept map is the judgment; the expansion is mechanical.** NIPA's trade
lines *are* NAICS wholesale (423x/424x) and retail (44x/45x) categories, and
NAICS itself defines what each distributes, so each line maps to the NAICS goods
ranges its own definition names and those expand to BEA detail through
`NAICS_to_BEA_Crosswalk_2017.csv`. Nothing is fitted to the 2017 column. The
generator keeps that judgment reviewable rather than buried in a CSV.

Measured against the published 2017 `F03000`: the crosswalk reaches **254 of the
258 populated commodities, 91% of gross mass**. The four it does not reach —
`211000` oil and gas, `212100` coal, `21311A`, `213111` — are all the
mining/utilities/construction branch, which is open question 2 and correctly
outside a trade crosswalk.

The plan's three concept spot-checks all land: drugs wholesalers → `325412`,
petroleum wholesalers → `324110`, motor vehicle dealers → `336111`/`336112`.
They were chosen before the map was written, which is the admissible order.

⚠️ **The crosswalk keys on the NIPA line name, not its series code** — that is
what the FBA carries. All 27 published trade names are unique within
`U50705BU1`, but `C42ND`/`C42NN` are published as bare "Durable goods
industries" / "Nondurable goods industries", names that recur at four levels, so
the method selects them by `Line` and renames them via `assign_fields` before
attribution.

**Still open on this branch:** the weights *within* each commodity set. The
crosswalk defines what a trade line can reach; how its value splits across those
commodities is Phase 3. Do not weight it on the 2017 `F03000` column itself —
