...
for (i, c), f in partial.items():
    print(
        f'  donor {i} -> commodity {c}: {f:6.1%} of ${before_vals[(i, c)]/M:,.0f}M '
...
print(
    f'aggregate intermediate-per-output intensity: '
...
for name, block in (
    ('donors (lose output)', donors),
    ('receivers (gain output)', receivers),
):
    ...
    print(
        f'  intensity quartiles: '
        f'{block["intensity"].quantile([0.25, 0.5, 0.75]).round(3).tolist()}'
    )

print('\nlargest receivers, intensity each:')
for code, row in (
    receivers.sort_values('output_moved', ascending=False).head(12).iterrows()
):
    print(
        f'  {code}: gained ${row.output_moved/M:>10,.0f}M output, '
...
for code, row in donors.sort_values('output_moved').head(12).iterrows():
    print(
        f'  {code}: lost ${-row.output_moved/M:>10,.0f}M output, '
        f'${-row.intermediate_moved/M:>10,.0f}M intermediates -> intensity {row.intensity:5.2f}'
    )

# Sanity: industry output identity after redef
x_gap = (
    (V_a.sum(axis=1) - (U_a.sum(axis=0) + (V_a.sum(axis=1) - U_a.sum(axis=0))))
    .abs()
    .max()
)
