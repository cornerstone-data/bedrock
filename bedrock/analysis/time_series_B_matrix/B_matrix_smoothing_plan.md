...
| **D16** | 1 → 2 bridge | could a facility vector carry the sections table 3-11 is not? | For every family outside table 3-11 that rides a vector into a facility-scope sector — 106.2 Mt in 2022 against 808.8 Mt for table 3-11 — the share landing where the GHGRP sees nothing, how far a facility vector would move the split, and **which subparts would be doing the moving**. The last is the finding: cement calcination, hydrogen and refinery process mass are not non-energy-use carbon | `non_combustion_correspondence.csv` — ⚠️ **opt-in, `--facility-data`**. Answers [#962](https://github.com/cornerstone-data/bedrock/issues/962): every family fails, so the facility method replaces the five table 3-11 sections and leaves the rest |
