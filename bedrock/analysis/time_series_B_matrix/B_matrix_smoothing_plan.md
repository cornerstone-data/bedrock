...
with no arguments; it writes the core tables and figures to `output/`, plus the
NEI combustion/process backcast (needs NEI via stewi). `--facility-data` adds
D14 and D15 and more tables (GHGRP + more NEI). Figures for the numbers quoted
here are from the run of 2026-09-18 on FBS vintage
...
| **D15** | 1 → 2 bridge | how much of table 3-11 could rest on facilities that actually reported, and where would that move the split? | GHGRP subpart C first, then NEI combustion SCCs for the facilities below GHGRP's threshold, deduplicated on `FRS_ID`; sector from the **facility** NAICS, never from the process code. Splits `purchased` from `self_supplied` fuel, because fuel that never changed hands — byproduct gas, and the lease and plant fuel of an oil and gas operation (#927) — is never a purchase and no Use row can carry it. Companion: `nei_combustion_process_backcast` restates pre-2021 combustion vs process coding from 2021/2022 twins without changing union levels or `fuel_class` (always written by `main`) | `facility_basis.csv`, `facility_combustion.csv` — ⚠️ **opt-in, `--facility-data`**; `nei_combustion_process_backcast.csv` — always; bounded by NEI, which trails GHGRP by a year |
