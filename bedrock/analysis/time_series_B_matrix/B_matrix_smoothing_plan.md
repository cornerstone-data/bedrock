...
| **D14** | 1 → 2 bridge | did we allocate a sector less fuel combustion than its facilities physically reported? | GHGRP subpart C (stationary fuel combustion, facility level, over the 25,000 tCO2e threshold) crosswalked from facility NAICS to BEA detail, as an annual **lower bound** on each sector's table 3-11. A sector below the floor in *every* year is a boundary definition difference; one that clears it in some years and breaches it in others has a volatility defect | `combustion_floor_test.csv`, `ghgrp_subpart_C.csv` — ⚠️ **opt-in, `--facility-data`**: the only diagnostic here that reaches outside the repository, downloading GHGRP per year through `stewi`. Covers 2017-2023; the reporting programme runs a year behind the span |
| **D15** | 1 → 2 bridge | how much of table 3-11 could rest on facilities that actually reported, and where would that move the split? | GHGRP subpart C first, then NEI combustion SCCs for the facilities below GHGRP's threshold, deduplicated on `FRS_ID`; sector from the **facility** NAICS, never from the process code. Splits `purchased` from `facility_derived` fuel, because byproduct gas is never a purchase and no Use row can carry it | `facility_basis.csv`, `facility_combustion.csv` — ⚠️ **opt-in, `--facility-data`**; bounded by NEI, which trails GHGRP by a year |
