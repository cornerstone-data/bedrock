...
Bedrock v0.2 is the first release in which **USEEIO and CEDA-US are merged** into a single, configurable U.S. national EEIO (Environmentally-Extended Input-Output) model. USEEIO is a US specific legacy EEIO model originally developed by the US EPA. CEDA-US (Comprehensive Environmental Data Archive) is the US portion of the CEDA Multi-Regional EEIO (MRIO) model, developed by Watershed. v0.2 brings in methodological contributions and improvements from both models, including a reworked sector schema, an updated approach to reconcile data years between the E matrix (environmental extensions) and B matrix (per-dollar emissions intensities), and a merged attribution model for per-sector greenhouse-gas (GHG) emissions.
