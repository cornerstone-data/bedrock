...
Bedrock v0.2 is the first release in which **USEEIO and CEDA-US are merged** into a single, configurable U.S. national EEIO (Environmentally-Extended Input-Output) model USEEIO is the U.S. Environmental Protection Agency's (EPA) legacy United States (US) EEIO model; CEDA-US (Comprehensive Environmental Data Archive) is the US portion of Watershed's proprietary Multi-Regional EEIO (MRIO) model. v0.2 brings in methodological contributions and improvements from both — a reworked sector schema, an updated approach to reconcile data years between the E matrix (environmental extensions) and B matrix (per-dollar emissions intensities), and a merged attribution model for per-sector greenhouse-gas (GHG) emissions — with every choice between the two lineages explicit as a typed configuration flag.
