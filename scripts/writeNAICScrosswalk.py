...
def load_naics_concordance(y1, y2):
    ...
    :return:
    """
    df_load = pd.read_csv(f'{externaldatapath}/{y1}_to_{y2}_NAICS.csv', dtype=str)
    # drop first rows
    ...
    naics_cw = df[[f'{y1} NAICS Code', f'{y2} NAICS Code']].rename(
        columns={
            f'{y1} NAICS Code': f'NAICS_{y1}_Code',
...
def update_naics_crosswalk():
    ...
    for i, ii in zip(years, years[1:]):
        ...
        df = df[['NAICS', f'NAICS_{ii}_Code']].rename(
            columns={'NAICS': f"NAICS_{i}_Code"}
        )
        dfm = dfm.merge(df, how='outer')

    # sort df
    dfm = (
        dfm.sort_values(['NAICS_2012_Code', 'NAICS_2007_Code'])
        .drop_duplicates()
        .reset_index(drop=True)
    )

    # save as csv
    dfm.to_csv(f"{mappingpath}/naics/NAICS_Crosswalk_TimeSeries.csv", index=False)
...
def merge_df_by_crosswalk_lengths(naics_cw, d, l):
    ...
    naics_cw = (
        d[f'NAICS_{l}']
        .assign(
            temp=d[f'NAICS_{l}'][f'NAICS_{l}'].str.extract(
                pat=f"({'|'.join(map(str, naics_cw[f'NAICS_{l - 1}']))})"
            )
        )
        .merge(
            naics_cw,
...
def write_annual_naics_crosswalk(year):
    ...
    :return:
    """
    cw = pd.read_csv(f'{externaldatapath}/2-6 digit_{year}_Codes.csv', dtype=str)
    ...
    cw2 = (
        pd.concat(
            [cw, pd.DataFrame(missing_parents, columns=[f'NAICS_{year}_Code'])],
            ignore_index=True,
        )
    ...
    for file_name in glob.glob(f'{crosswalkpath}/NAICS_Crosswalk_*.csv'):
        ...
        if not any(s in file_name for s in ('StatCan', 'BEA')):
            ...
            missing_naics = df[
                (~df[f'NAICS_{year}_Code'].isin(common[f'NAICS_{year}_Code']))
            ]
            # append to df list
            missing_naics_df_list.append(missing_naics)
    # concat df list and drop duplications
    missing_naics_df = (
        pd.concat(missing_naics_df_list, ignore_index=True, sort=False)
        .drop_duplicates()
        .reset_index(drop=True)
    )

    # add additional cddpath sectors - defined in HIO github repo
    cdd_sectors = pd.DataFrame({f"NAICS_{year}_Code": ['5629202', '5629203']})
    ...
    # save as csv
    naics_cw.to_csv(f"{mappingpath}/naics/NAICS_{year}_Crosswalk.csv", index=False)
...
def write_sector_name_crosswalk():
    ...
    naics_2012 = pd.read_excel(
        ...
        skiprows=[0],
        dtype=str,
    )[['NAICS_2012_Code', 'NAICS_2012_Name']]

    cols_2017 = ['NAICS_2017_Code', 'NAICS_2017_Name', 'NAICS_2017_Description']
    naics_2017 = pd.read_excel(
        "https://www.census.gov/naics/2017NAICS/2017_NAICS_Descriptions.xlsx",
        names=cols_2017,
        dtype=str,
    )[['NAICS_2017_Code', 'NAICS_2017_Name']]
    ...
    for y in ['2012', '2017']:
        ...
        df[f"NAICS_{y}_Code"] = (
            df[f"NAICS_{y}_Code"]
            .str.split('\s*-\s*')
            .apply(lambda x: list(range(int(x[0]), int(x[-1]) + 1)))
        )
